...
for (z in 1:length(cutoffs)){
  ...
  mcl_dirs <- paste0(unique(mcl_gene_dist_df$metacluster))
  for(dir in mcl_dirs){
  ...
  # for (n in 1:length(mcl_dirs)){
    clusters <- str_split(rownames(mcl_df[which(mcl_df$metacluster == n),]),'--')
  ...
  ggsave(paste0(cutoff_dir, analysis_dirs$metacluster_analysis, date, '_average_log_expression_over_time.png'), plot = all_mcl_log_expr_plot, width = 10, height = 8)
  ...
  # ggsave(paste0(cutoff_dir, analysis_dirs$metacluster_analysis, date, '_average_expression_over_time.png'), plot = all_mcl_expr_plot, width = 10, height = 8)
