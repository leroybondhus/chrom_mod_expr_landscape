...
gene_lists <- foreach(k = 1:length(structure_names), .errorhandling = 'pass') %dopar% {
  ...
  struct = str_replace_all(struct_name, "[ /]", "_") 
    ...
    if(nrow(chrom_mod_normalized_expr) > 2){
      ...
      cluster0 <- rows_meta[strtoi(row.names(struct_matrix[which(apply(struct_matrix, 1, max)-apply(struct_matrix, 1, min)<=0.5),])),] ## NEED TO FIX BC ALL NA RIGHT NOW
...
for(i in 1:length(mcl_time_modest_gene_counts)){
  print(max(mcl_time_modest_gene_counts[[i]]))
  if (max(mcl_time_modest_gene_counts[[i]]) > 2){
    count = count + 1
  }
}
print(count)
