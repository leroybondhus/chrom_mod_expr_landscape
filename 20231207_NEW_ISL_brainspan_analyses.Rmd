...
for (z in 1:length(cutoffs)){
  ...
  for (i in 1:length(mcl_dirs)){
    ...
    # determine if genes were expressed in each tissue in a metacluster
    ...
    for (j in 1:nrow(sub_df)){
      ...
      grid_df[which(is.element(rownames(grid_df), cluster_epigenes)),which(colnames(grid_df)==struct)] = 1
    }
    
    # format grid_df for plotting
    grid_df$gene <- rownames(grid_df)
    melted_grid_df <- melt(grid_df, id.vars = 'gene', value.name = 'is_expr')
