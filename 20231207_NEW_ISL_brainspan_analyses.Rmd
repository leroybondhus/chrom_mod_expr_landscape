...
iagc_gs_sim_mat <- matrix(nrow = length(unique_pairs), ncol = length(unique_pairs), dimnames = list(unique_pairs, unique_pairs))
...
iagc_gs_sim_mat_modest <- foreach(i = 1:length(unique_pairs_modest), .combine = 'rbind') %:%
  foreach(j = 1:length(unique_pairs_modest), .combine = 'c') %dopar% {
    first <- str_split(unique_pairs_modest[i], '--')[[1]]
    second <- str_split(unique_pairs_modest[j], '--')[[1]]
    jaccard(gene_lists[[first[1]]][[first[2]]][[first[3]]],gene_lists[[second[1]]][[second[2]]][[second[3]]])
  }

rownames(iagc_gs_sim_mat_modest) <- unique_pairs_modest
colnames(iagc_gs_sim_mat_modest) <- unique_pairs_modest

write.csv(iagc_gs_sim_mat, paste0(dirs$table_cluster_analysis, date, "_iagc_gs_sim_mat.csv"))
```
