...
ptm <- proc.time()
unique_pairs <- unique(paste(all_genes_GO_results$Structure, all_genes_GO_results$Cutoff, all_genes_GO_results$Cluster, sep = '--'))

# iagc_time_sim_mat <- matrix(nrow = length(unique_pairs), ncol = length(unique_pairs), dimnames = list(unique_pairs, unique_pairs))

# iagc_time_sim_mat <- foreach(i = 1:5, .combine = 'rbind') %:%
for (a in 1:500){
  for (b in 1:500){
  # foreach(i = 1:5, .combine = 'c') %dopar% {
    first <- str_split(unique_pairs[a], '--')[[1]]
    # iagc_time_sim_vec <- vector(mode = "numeric", length = 5)
    second <- str_split(unique_pairs[b], '--')[[1]]
    ...
    if(first[[1]] == second[[1]] & first[[2]] == second[[2]] & first[[3]]!= second[[3]]){
      ...
      for (i in 1:length(first_matches)){
        if (is.na(match(first_matches[i], pcw_pairs))){
          pcw_pairs <- append(pcw_pairs,first_matches[i])
        }
      }
      ...
      # iagc_time_sim_vec[b] = 
      iagc_time_sim_mat[a,b] = cor(new_first_expr, new_second_expr)
    }
  }
  # iagc_time_sim_vec
}
...
# write.csv(iagc_time_sim_mat, paste0(dirs$table_cluster_analysis, date, "_iagc_time_sim_mat.csv"))
```


```{r, inter-anatomic-gene-cluster-gene_set-similarity-matrix}
iag_gs_sim_mat <- matrix(nrow = length(unique_pairs), ncol = length(unique_pairs), dimnames = list(unique_pairs, unique_pairs))
...
for (i in 1:length(unique_pairs)){
  first <- str_split(unique_pairs[i], '-')[[1]]
  for (j in 1:length(unique_pairs)){
    second <- str_split(unique_pairs[j], '-')[[1]]
    iag_gs_sim_mat[i,j] = jaccard(gene_lists[[first[1]]][[first[2]]][[first[3]]], gene_lists[[second[1]]][[second[2]]][[second[3]]])
  }
}

write.csv(iag_gs_sim_mat, paste0(dirs$table_cluster_analysis, date, "_iag_gs_sim_mat.csv"))
```
