...
# BiocManager::install("DOSE") # for GO visualization
```
...
cutoff_dirs <- list(high_cutoff = "./results/high_res_cutoff_0.2/",
             modest_cutoff = "./results/modest_res_cutoff_0.4/")
...
```{r, calculate gene expression correlation - DON"T NEED}
...
## IS THIS WARNING OKAY? Warning: scheduled cores 2, 4, 9 did not deliver results, all values of the jobs will be affected
  ...
  cutoff_gene_lists[[cutoff_name]] <- foreach(k = 1:length(structure_names), .errorhandling = 'pass') %dopar% {
    ...
    struct = str_replace_all(struct_name, "[ /]", "_") 
      ...
      if(nrow(chrom_mod_normalized_expr) > 2){
        ...
        pcw <- sorted_cols$age[struct_index]
        pcw_range <- c(pcw[1], pcw[length(pcw)])
        window_size <- 10
        pcw_weights <- vector("numeric", length = length(pcw))
        # pcw_weights <- vector("numeric", length = (pcw_range[2]/window_size))
        i = pcw_range[1]
        while(i <= pcw_range[2]){
        ...
        hcl_df$pcw <- sorted_cols$age[struct_index]
        ...
        struct_cluster_assignment <- vector("character", nrow(sub_matrix)) 
...
for (z in 1:length(cutoffs)){
  ...
  cutoff_gene_list <- unlist(unlist(cutoff_gene_lists[[cutoff_name]], recursive = FALSE)[((1:length(unlist(gene_lists, recursive = FALSE))) %%2)!=0], recursive = FALSE)
  
  unique_pairs <- names(cutoff_gene_list)[grep(paste0(names(cutoffs)[z]), names(cutoff_gene_list))]
  
  # split(1:100, ceiling(seq_along(1:100)/20))
    ...
    for (d in 1:length(parallel_blocks[[a]])){
      ...
      # foreach(b = 1:length(unique_pairs), .combine = 'c') %dopar% { # replace with for loop
        ...
        if(first[[1]] == second[[1]] & first[[2]] == second[[2]] & first[[3]]!= second[[3]]){
          ...
          # perform time-weighted correlation - NEED TO FIX
          pcw <- sorted_cols$age[struct_index]
          pcw_range <- c(pcw[1], pcw[length(pcw)])
          window_size <- 10
          pcw_weights <- vector("numeric", length = length(pcw))
          ...
          result <- cov.wt(new_first_expr, new_second_expr, wt = pcw_weights, cor = TRUE)$cor
...
hc_time_modest <- hclust(as.dist(1-(cutoffs_iagc_time_sim_mat[[cutoff_name]]+1)/2), "ave")
png(filename = paste0(cutoff_dir, analysis_dirs$metacluster_analysis, struct, "/", date, "_", struct, "_clustering_dendrogram(cutoff=", cutoff, ").png"), width = 960, height = 480)
plot(hc_time_modest, main = "metaclusters for iagc_time modest resolution (cutoff = 0.4)", cex=0.25)
rect.hclust(hc_time_modest, h=cutoff)
dev.off()
...
for (i in 1:length(mcl_time_modest_gene_counts)){
  mcl_time_modest_gene_dist[[i]] = as.vector(table(mcl_time_modest_gene_counts[[i]]), mode = "list")
}
...
all_mcl_log_expr_plot <- ggplot(all_mcl_avg_expr, aes(x = log10(pcw), y = expr)) +
            geom_point() + geom_line() +
...
ggsave(paste0(dirs$figure_cluster_analysis, 'all_genes_modest_res_cutoff_0.4/', date, '_average_log_expression_over_time.png'), plot = all_mcl_log_expr_plot, width = 10, height = 8)

all_mcl_expr_plot <- ggplot(all_mcl_avg_expr, aes(x = pcw, y = expr)) +
            geom_point() + geom_line() +
            facet_wrap(~metacluster) +
      theme(legend.position = "none") +
            ggtitle('metacluster average expression over time') + 
            labs(x = "pcw", y = "normalized expr") + 
           geom_vline(xintercept = 40, color = "red") + geom_vline(xintercept = 52*15, color = "red") + geom_vline(xintercept = 52*10, color = "red")
...
ggsave(paste0(dirs$figure_cluster_analysis, 'all_genes_modest_res_cutoff_0.4/', date, '_average_expression_over_log_developmental_time.png'), plot = develop_mcl_log_expr_plot, width = 10, height = 8)
...
ggsave(paste0(dirs$figure_cluster_analysis, 'all_genes_modest_res_cutoff_0.4/', date, '_all_metaclusters_gene_tissue_freq_hist.png'), plot = mcl_hist, width = 10, height = 8) 
# summary of gene count - genes in each cluster, genes in each metacluster
```


```{r,  inter-anatomic-gene-cluster-time-similarity-matrix - modest resolution}
# run GO analysis on genes belonging to at least 3-5 tissues
mcl_gene_list <- vector("list", length(mcl_dirs))
names(mcl_gene_list) <- mcl_dirs
for (i in 1:length(mcl_dirs)){
  mcl_gene_list[[mcl_dirs[i]]] <- names(mcl_time_modest_gene_counts[[mcl_dirs[i]]][which(mcl_time_modest_gene_counts[[mcl_dirs[i]]]>=3)])
}
...
all_mcl_GO_results <- foreach(i = 1:length(mcl_gene_list), .combine = rbind) %dopar% {
  if(length(mcl_gene_list[[i]])>0){
    ...
    if (nrow(saved_enrich) > 0){
      ...
      p_emapplot <- emapplot(term_sim_matrix, showCategory = 15, layout = "kk", circular = FALSE,
         color.params = list(edge = TRUE, category = 'firebrick', gene = 'black'), 
         cex.params = list(category_node = 0.6, gene_node = 0.6, category_label = 1, gene_label = 0.7),
         title = paste("Enrichment emapplot -", mcl_dirs[i])) +
              custom_theme_2() +
              ggtitle(paste("Enrichment emapplot -", mcl_dirs[i]))
      # ggsave(paste0(dirs$figure_cluster_analysis, 'all_genes_modest_res_cutoff_0.4/', mcl_dirs[i],'/', date, '_', mcl_dirs[i],'_emapplot.png'), p_emapplot, width = 15, height = 10, units = "in", bg = "white")
      ggsave(paste0(dirs$figure_cluster_analysis, 'all_genes_modest_res_cutoff_0.4/', mcl_dirs[i],'/', date, '_', mcl_dirs[i],'_combined_plot.png'), grid.arrange(p_cnetplot, p_emapplot), width = 15, height = 10, units = "in", bg = "white")
...
write.csv(all_mcl_GO_results, paste0(dirs$table_cluster_analysis, date, "_all_mcl_GO_results.csv"))
```
...
```{r, inter-anatomic-gene-cluster-gene_set-similarity-matrix}
