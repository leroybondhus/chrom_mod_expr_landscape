...
gene_lists <- foreach(k = 1:length(structure_names), .errorhandling = 'pass') %dopar% {
  ...
  # if (length(struct_index) > 1){ - did prefiltering above
    ...
    # mean (normalized to [0-1])  expression per time point for each gene (need dataframe format to access row names when creating gene lists)
...
# iagc_time_sim_mat <- matrix(nrow = length(unique_pairs), ncol = length(unique_pairs), dimnames = list(unique_pairs, unique_pairs))
...
iagc_time_sim_mat_modest <- foreach(a = 1:length(unique_pairs_modest), .combine = 'rbind') %:%
  foreach(b = 1:length(unique_pairs_modest), .combine = 'c') %dopar% {
    first <- str_split(unique_pairs_modest[a], '--')[[1]]
    second <- str_split(unique_pairs_modest[b], '--')[[1]]
...
rownames(iagc_time_sim_mat_modest) <- unique_pairs_modest
colnames(iagc_time_sim_mat_modest) <- unique_pairs_modest
...
plot(hc_time_modest,labels=FALSE, main = "metaclusters for iagc_time modest resolution (cutoff = 0.4)")
rect.hclust(hc_time_modest, h=0.2)
```
...
iagc_time_sim_mat_high <- foreach(a = 1:length(unique_pairs_high), .combine = 'rbind') %:%
  foreach(b = 1:length(unique_pairs_high), .combine = 'c') %dopar% {
    first <- str_split(unique_pairs_high[a], '--')[[1]]
    second <- str_split(unique_pairs_high[b], '--')[[1]]
    # For all clusters within the same anatomic group, just set inter-anatomic cluster dist = 0
    if(first[[1]] == second[[1]] & first[[2]] == second[[2]] & first[[3]]!= second[[3]]){
      result = 0
    } else{
...
rownames(iagc_time_sim_mat_high) <- unique_pairs_high
colnames(iagc_time_sim_mat_high) <- unique_pairs_high
...
mcl_time_modest <- cutree(hc_time_modest, h=0.2)

# for each metacluster, get the union of all genes across all clusters in a given metacluster, 
mcl_time_modest_genes <- vector("list", max(mcl_time_modest))
names(mcl_time_modest_genes) <- paste0('metacluster',1:max(mcl_time_modest))
...
for (i in 1:max(mcl_time_modest)){
  ...
  mcl_time_modest_genes[[i]] <- mcl_genes
}
