...
for (z in 1:length(cutoffs)){
  ...
  # spaghetti plot for all metaclusters?? - try later
  ...
  for (i in 1:length(mcl_dirs)){
    ...
    for (j in 1:nrow(sub_df)){
      ...
      grid_df[which(is.element(rownames(grid_df), cluster_epigenes)),which(colnames(grid_df)==struct)] = 1 # similarity = 1
    }
    ...
    matched_epigene_indices <- which(is.element(rownames(grid_df),all_top_epigenes))
    sub_grid_df <- grid_df[matched_epigene_indices,]
    sub_grid_df <- sub_grid_df[1:40,]
    
    # format grid_df for plotting
    sub_grid_df$gene <- rows_meta$gene_symbol[which(is.element(rows_meta$ensembl_gene_id, rownames(sub_grid_df)))] # use gene symbol instead of ensembl id
