...
# library(wCorr) # don't need

# BiocManager::install("DOSE") # for GO visualization
# install.packages("wCorr") # don't need
```
...
num_struct_timepoints <- vector("integer", num_unique_structures)
...
for (a in 1:length(cutoffs)){
  ...
  cutoff_gene_lists[[cutoff_name]] <- foreach(k = 1:length(structure_names), .errorhandling = 'pass') %dopar% {
    ...
    struct <- str_replace_all(struct_name, "[ /]", "_") 
      ...
      protein_coding_sig_genes <- match(all_genes_significant, protein_coding_genes)
      protein_coding_sig_genes <- protein_coding_genes[protein_coding_sig_genes[!is.na(protein_coding_sig_genes)]]
      ...
      if(nrow(chrom_mod_normalized_expr) > 2){
        ...
        for(i in 1:n_clusters){
          all_gene_lists[[i]] = protein_coding_sig_genes[which(struct_cluster_assignment == cluster_names[i])]
        }
...
```{r, GO analysis setup}
gene_universe <- rows_meta$ensembl_gene_id[which(is.element(rows_meta$ensembl_gene_id, protein_coding_genes))] # protein-coding genes in brainspan
...
for (z in 1:length(cutoffs)){
  ...
  plot(hc_meta, main = paste0("metaclusters for iagc_time_similarity matrix resolution (cutoff=", cutoff, ")"), cex=0.25)
  rect.hclust(hc_meta, h=cutoff)
  dev.off()
  
  
  mcl <- cutree(hc_meta, h=cutoff) # USING 0.1 METACLUSTER CUTOFF for both clustering cutoffs
  ...
  write.csv(clusters_in_mcl, paste0(cutoff_dir, analysis_dirs$metacluster_analysis, date, "_cluster_count_in_mcl(cutoff=0.2).csv"))
  ...
  # for (n in 1:length(mcl_dirs)){
    ...
    if (!is.null(ncol(new_gene_expr_df))){ # only 1 cluster
      test <- melt(new_gene_expr_df, id = rownames(new_gene_expr_df), value.name = "expr", varnames = c("pcw", "sample"))
      test$is_ave=F
      ...
      gg_spaghetti <- ggplot(test, aes(x = pcw, y = expr, color = sample, group = sample)) + 
  ...
  # spaghetti plot for all metaclusters
  ...
  for (i in 1:length(mcl_dirs)){
    ...
    matched_modifiers <- match(chromatin_modifiers, names(mcl_gene_counts[[metacluster]]))
    # remove NA values
    matched_modifiers <- matched_modifiers[!is.na(matched_modifiers)]
  ...
  for (i in 1:length(mcl_dirs)){
    mcl_gene_list[[mcl_dirs[i]]] <- names(mcl_gene_counts[[mcl_dirs[i]]][which(mcl_gene_counts[[mcl_dirs[i]]]>=3)])
  }
...
# distribution of epigenes (brainregions on y-axis, counts of epigenes on x-axis)
...
which <- which(is.element(gene_universe, names(mcl_gene_counts[[1]]))) # use all genes in a given metacluster
temp_chrom <- onto_maps$HPO_OMIM_GENE$map[which,]
temp_not_chrom <- onto_maps$HPO_OMIM_GENE$map[-which,] # can modify -which for some other contrast gene set - all other genes (setdiff function) - use all protein coding genes
