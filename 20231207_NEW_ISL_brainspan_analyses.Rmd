...
for (z in 1:length(cutoffs)){ # CODE DOESNT WORK FOR Z=1 (cutoff=0.4) RIGHT NOW
  ...
  angio_mcl4 <- angio_imputed_mcl_avg_expr[which(angio_imputed_mcl_avg_expr$metacluster==angio_mcls[1]),]
  angio_mcl4 <- data.frame(pcw = angio_mcl4$pcw, sample = 'average', expr = angio_mcl4$expr, metacluster = angio_mcl4$metacluster) # need columns to match spaghetti df
  
  angio_mcl31 <- angio_imputed_mcl_avg_expr[which(angio_imputed_mcl_avg_expr$metacluster==angio_mcls[2]),]
  angio_mcl31 <- data.frame(pcw = angio_mcl31$pcw, sample = 'average', expr = angio_mcl31$expr, metacluster = angio_mcl31$metacluster)
  
  angio_gg_spaghetti <- ggplot(angio_spaghetti_df, aes(x = pcw, y = expr, color = metacluster, group = sample)) + 
        geom_line(alpha = 0.3) + theme_bw() + 
        geom_line(data=angio_mcl4, alpha=1, linewidth=2) +
        geom_line(data=angio_mcl31, alpha=1, linewidth=2)+
        # theme(legend.position = "none") + 
  ...
  # top_epigenes <- vector('character', length = 5*length(mcl_dirs))
  top_epigenes <- vector('character', length = 5*length(interest_mcls))
  # find top 5 epigenes in each metacluster
  # for (i in 1:length(mcl_dirs)){
  for (i in 1:length(interest_mcls)){
    ...
    top_epigenes[(5*i-4):(5*i)] <- names(sorted_mcl_epigenes)[1:5]
  }
  ...
  # for (i in 1:length(mcl_dirs)){
    ...
    sub_df <- mcl_df[which(mcl_df$metacluster==i),]
    ...
    mcl_expr <- all_mcl_avg_expr$expr[which(all_mcl_avg_expr$metacluster == mcl_dirs[i])]
    names(mcl_expr) <- all_mcl_avg_expr$pcw[which(all_mcl_avg_expr$metacluster == mcl_dirs[i])]
    ...
    # subset grid_df for top 40 epigenes in a metacluster
    # sorted_mcl_genes <- sort(mcl_gene_counts[[mcl_dirs[i]]], decreasing = TRUE)
    # sorted_mcl_epigenes <- sorted_mcl_genes[which(is.element(names(sorted_mcl_genes),rownames(grid_df)))] 
    # top_epigenes <- names(sorted_mcl_epigenes)[which(sorted_mcl_epigenes >= 4)]
    
    matched_epigene_indices <- match(all_top_epigenes, rownames(grid_df))
    sub_grid_df <- grid_df[matched_epigene_indices,]
    # sub_grid_df <- sub_grid_df[1:50,]
    ...
    p_grid <- ggplot(melted_grid_df, aes(variable, gene, fill=is_expr)) + 
      geom_tile() + 
      ggtitle(paste0(mcl_dirs[i], ' epigenes-tissue grid plot')) + 
      labs(x = "tissue", y = "gene") + 
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
      scale_fill_gradientn(colors = custom_palette, limits = c(-1, 2), breaks = c(-1, 0, 1, 2), labels = c("-1", "0", "1", "2")) 
