...
#which <- which(df$first_last_single_middle=="last"  & grepl("control_set|all_chrom",df$chrom_subset_name) )

which <- which(df$is_last & grepl("control_set|all_chrom",df$chrom_subset_name) )

ggplot(df[which,],aes(x=log10(exon_count+1),color=chrom_subset_name))+
  geom_step(aes(y=..y..),stat="ecdf")
  # geom_density()#+
  # facet_grid(. ~ chrom_subset_name)
filename <- paste0(dirs$figures, "exon_count__ecdf.png")
ggsave(filename = filename, height = 4, dpi=250)

  
ggplot(df[which,],aes(x=exon_length/transcript_length,color=chrom_subset_name))+
  geom_step(aes(y=..y..),stat="ecdf") #geom_histogram()
filename <- paste0(dirs$figures, "last_exon_div_by_transcript_len.png")
ggsave(filename = filename, height = 4, dpi=250)


ggplot(df[which,],aes(y=exon_length/transcript_length, x=log10(transcript_length), color=chrom_subset_name))+
  geom_point(aes(alpha=chrom_subset_name),size=0.5)+
  scale_alpha_manual(name = "chrom_subset_name", values = c(.4, .05))+
  geom_density_2d() +
  xlim(2.5,4.5)+
 # geom_smooth(method='lm', formula= x~y)+
  facet_grid(chrom_subset_name ~ . )
filename <- paste0(dirs$figures, "last_exon_div_by_transcript_len_v_log10transcript_len.png")
ggsave(filename = filename, height = 5, dpi=250)

which <- which(df$is_last & grepl("control_set|all_chrom",df$chrom_subset_name) & 
                 df$transcript_length > 10^2.5 & df$transcript_length < 10^4.5)
df$log10_transcript_length_bin <- cut(log10(df$transcript_length),breaks=seq(0,10,by=0.5))



ggplot(df[which,],aes(x=exon_length/transcript_length,color=chrom_subset_name))+
  geom_density()+ #geom_histogram()+
  facet_grid(. ~ log10_transcript_length_bin )
filename <- paste0(dirs$figures, "exonlength_div_tranlen_density.png")
ggsave(filename = filename, height = 4, dpi=250)

ggplot(df[which,],aes(x=exon_length/transcript_length,color=chrom_subset_name))+
  geom_step(aes(y=..y..),stat="ecdf")+ #geom_histogram()+
  facet_grid(. ~ log10_transcript_length_bin )
filename <- paste0(dirs$figures, "exonlength_div_tranlen_density_ecdf.png")
ggsave(filename = filename, height = 4, dpi=250)


ggplot(df[which,],aes(x=exon_length/transcript_length,linetype=chrom_subset_name, color=log10_transcript_length_bin))+
  geom_step(aes(y=..y..),stat="ecdf") #+ #geom_histogram()+
filename <- paste0(dirs$figures, "exonlength_div_tranlen_density_ecdf_one_panel.png")
ggsave(filename = filename, height = 4, dpi=250)



ggplot(df[which,],aes(y=log10(exon_count+1), x=log10(transcript_length), color=chrom_subset_name))+
  geom_point(aes(alpha=chrom_subset_name),size=0.5)+
  scale_alpha_manual(name = "chrom_subset_name", values = c(.4, .05))+
  geom_density_2d() +
  facet_grid(chrom_subset_name ~ .)
filename <- paste0(dirs$figures, "exon_count_v_log10transcript_len.png")
ggsave(filename = filename, height = 5, dpi=250)


## for each transcript size bin - look at distribution of exon count
which <- which(df$is_last & grepl("control_set|all_chrom",df$chrom_subset_name) & 
                 df$transcript_length > 10^2.5 & df$transcript_length < 10^4.5)
df$log10_transcript_length_bin <- cut(log10(df$transcript_length),breaks=seq(0,10,by=0.5))
ggplot(df[which,],aes(x=log10(exon_count+1), color=chrom_subset_name))+
  geom_density()+
  facet_grid(. ~ log10_transcript_length_bin )
filename <- paste0(dirs$figures, "exon_count_by_transcript_size_bin.png")
ggsave(filename = filename, height = 4, dpi=250)
...
ggplot(df[which,],aes(x=log10(exon_count+1), color=chrom_subset_name))+
  geom_step(aes(y=..y..),stat="ecdf")+
  facet_grid(. ~ log10_transcript_length_bin )
filename <- paste0(dirs$figures, "exon_count_by_transcript_size_bin_ecdf.png")
ggsave(filename = filename, height = 4, dpi=250)

ggplot(df[which,],aes(x=log10(exon_count+1),linetype=chrom_subset_name , color=log10_transcript_length_bin))+
  geom_step(aes(y=..y..),stat="ecdf")
filename <- paste0(dirs$figures, "exon_count_by_transcript_size_bin_ecdf_one_panel.png")
ggsave(filename = filename, height = 4, dpi=250)
