...
spec_biosamples_list <- foreach(spec_gene_block=spec_gene_blocks) %dopar% {
  ...
  for(g in spec_gene_block){
    spec_biosamples[[g]] <- list(colnames = colnames(weighted_zscores[g, which(weighted_zscores[g,] > 2),drop=F]))
...
spec_biosamples_list_2 <- foreach(spec_biosamples=spec_biosamples_list) %dopar% {
  for(i in 1:length(spec_biosamples)){
...
cutpoints <- c(0.8,0.4,0.2)
myhcl <- as.data.frame(cutree(hc, h=cutpoints))
colnames(myhcl) <-  paste0("cut_",cutpoints,"_cluster")#  c("cut_0.4_cluster","cut_0.2_cluster")
myhcl$gene <- rownames(myhcl)

genes_in_order <- c()

for(cutpoint_cluster in paste0("cut_",cutpoints,"_cluster")){
  temp_df <- data.frame(sort(table(myhcl[[cutpoint_cluster]]),decreasing = T))
  colnames(temp_df) <- c("cluster", str_replace(cutpoint_cluster,"cut_","count_"))
  myhcl <- merge(myhcl, temp_df, by.x=cutpoint_cluster, by.y="cluster")
}
# temp_df <- data.frame(sort(table(myhcl$cut_0.4_cluster),decreasing = T))
# colnames(temp_df) <- c("cluster", "count_0.4_cluster")
# myhcl <- merge(myhcl, temp_df, by.x="cut_0.4_cluster", by.y="cluster")
# 
# temp_df <- data.frame(sort(table(myhcl$cut_0.2_cluster),decreasing = T))
# colnames(temp_df) <- c("cluster", "count_0.2_cluster")
# myhcl <- merge(myhcl, temp_df, by.x="cut_0.2_cluster", by.y="cluster")

# myhcl <- myhcl[order(myhcl$count_0.4_cluster, myhcl$cut_0.4_cluster, 
#                      myhcl$count_0.2_cluster, myhcl$cut_0.2_cluster,decreasing = T),]

myhcl <- myhcl[order(myhcl$count_0.8_cluster, myhcl$cut_0.8_cluster,
                     myhcl$count_0.4_cluster, myhcl$cut_0.4_cluster, 
                     myhcl$count_0.2_cluster, myhcl$cut_0.2_cluster,decreasing = T),]
...
# result_df <- result_df[-c(6,15,21:22,24:26,33,45:47,51:55,118:135),]
# rownames(result_df) <- 1:nrow(result_df)
...
png(filename=paste0(dirs$figures,"heatmap_ontologic_ordering_zscores.png"),
    height = 30, width=24, res = 280,unit="in")
