...
which_R <- which(result_df$name=="striated muscle tissue" & result_df$subsumed_by=="")+1
which_L <- max(which(result_df$subsumed_by=="")[which(result_df$subsumed_by=="") < which(result_df$name=="striated muscle tissue")])
...
png(filename=paste0(dirs$figures,"full__heatmap_ontologic_ordering_zscores__muscle.png"),
    height = 5, width=4, res = 250,unit="in")
temp_mat4 <- temp_mat3[subset_genes_in_order,subset_columns_in_order]
colnames(temp_mat4) <- make.unique(str_replace(colnames(temp_mat4),"\\.\\.\\.","."))
colnames(temp_mat4) <- make.unique(str_remove(colnames(temp_mat4),"\\.\\..*"))
Heatmap(temp_mat4,
...
temp_onto_names <- c("immune system", "nervous system", "intestine", "striated muscle tissue")
