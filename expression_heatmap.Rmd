...
# which_cols <- setdiff(which_cols,
#                       sample(onto$ehh[[which(onto$name=="immune system")]],
#                              floor(0.9*length(onto$ehh[[which(onto$name=="immune system")]])) ) 
#                       )
...
cutoff_point2global_z = qnorm(0.9)
  ...
  for(g in spec_gene_block){
    ...
    spec_biosamples[[g]] <- list(colnames = colnames(
      weighted_zscores[g, which(weighted_zscores[g,] > qnorm(0.95)  & ### qnorm(0.95) = Zscore ~ 1.64
                            temp_mat[g,]-weighted_means[g] > 0.5  ), ## enforce some delta
                       drop=F])) 
...
# sink(file=paste0(dirs$results,"spec_biosamples_allgenes.txt"))
...
spec_biosamples <- spec_biosamples_list[gene_sets$human_chromatin_modifiers$gene_subsets$all_chrom_modifiers]
...
# result_df <- result_df[-c(3,6:8,10:18,21:29,33:38,44,61:72,88:92,107:108,127:130,174:190,208:235,306:351),]
# rownames(result_df) <- 1:nrow(result_df)
...
```{r NOTES}
### TO DO
### Look at mostly ubiquitous genes
### Look at phenotypes of ubiquitous gene syndromes
...
#     height = 24, width=24, res = 280,unit="in")
...
# which_R <- which(result_df$name=="immune system" & result_df$subsumed_by=="")+1
# which_L <- max(which(result_df$subsumed_by=="")[which(result_df$subsumed_by=="") < which(result_df$name=="immune system")])
# 
# subset_genes_in_order <- genes_in_order$gene_name[which(genes_in_order$result_df_row < which_R &
#                                                           genes_in_order$result_df_row > which_L)]
# subset_columns_in_order <- cols_in_order$columnames[which(cols_in_order$result_df_row < which_R &
...
#                         max(temp_mat3[genes_in_order$gene_name,cols_in_order$columnames ])),
#                       c("cyan4","cyan","lightcyan","white","pink", "red3", "darkred","black"))
# png(filename=paste0(dirs$figures,"full__heatmap_ontologic_ordering_zscores__immune.png"),
#     height = 6, width=4, res = 250,unit="in")
# temp_mat4 <- temp_mat3[subset_genes_in_order,subset_columns_in_order]
# colnames(temp_mat4) <- make.unique(str_replace(colnames(temp_mat4),"\\.\\.\\.","."))
# colnames(temp_mat4) <- make.unique(str_remove(colnames(temp_mat4),"\\.\\..*"))
# Heatmap(temp_mat4,
...
# 
# 
# which_R <- which(result_df$name=="nervous system" & result_df$subsumed_by=="")+1
# which_L <- max(which(result_df$subsumed_by=="")[which(result_df$subsumed_by=="") < which(result_df$name=="nervous system")])
# 
# subset_genes_in_order <- genes_in_order$gene_name[which(genes_in_order$result_df_row < which_R &
#                                                           genes_in_order$result_df_row > which_L)]
# subset_columns_in_order <- cols_in_order$columnames[which(cols_in_order$result_df_row < which_R &
...
#                         max(temp_mat3[genes_in_order$gene_name,cols_in_order$columnames ])),
#                       c("cyan4","cyan","lightcyan","white","pink", "red3", "darkred","black"))
# png(filename=paste0(dirs$figures,"full__heatmap_ontologic_ordering_zscores__nervous.png"),
#     height = 16, width=8, res = 250,unit="in")
# temp_mat4 <- temp_mat3[subset_genes_in_order,subset_columns_in_order]
# colnames(temp_mat4) <- make.unique(str_replace(colnames(temp_mat4),"\\.\\.\\.","."))
# colnames(temp_mat4) <- make.unique(str_remove(colnames(temp_mat4),"\\.\\..*"))
# Heatmap(temp_mat4,
...
# 
# 
# which_R <- which(result_df$name=="intestine" & result_df$subsumed_by=="")+1
# which_L <- max(which(result_df$subsumed_by=="")[which(result_df$subsumed_by=="") < which(result_df$name=="intestine")])
# 
# subset_genes_in_order <- genes_in_order$gene_name[which(genes_in_order$result_df_row < which_R &
#                                                           genes_in_order$result_df_row > which_L)]
# subset_columns_in_order <- cols_in_order$columnames[which(cols_in_order$result_df_row < which_R &
...
#                         max(temp_mat3[genes_in_order$gene_name,cols_in_order$columnames ])),
#                       c("cyan4","cyan","lightcyan","white","pink", "red3", "darkred","black"))
# png(filename=paste0(dirs$figures,"full__heatmap_ontologic_ordering_zscores__intestine.png"),
#     height = 6, width=5, res = 250,unit="in")
# temp_mat4 <- temp_mat3[subset_genes_in_order,subset_columns_in_order]
# colnames(temp_mat4) <- make.unique(str_replace(colnames(temp_mat4),"\\.\\.\\.","."))
# colnames(temp_mat4) <- make.unique(str_remove(colnames(temp_mat4),"\\.\\..*"))
# Heatmap(temp_mat4,
...
# 
# 
# which_R <- which(result_df$name=="musculature" & result_df$subsumed_by=="")+1
# which_L <- max(which(result_df$subsumed_by=="")[which(result_df$subsumed_by=="") < which(result_df$name=="musculature")])
# 
# subset_genes_in_order <- genes_in_order$gene_name[which(genes_in_order$result_df_row < which_R &
#                                                           genes_in_order$result_df_row > which_L)]
# subset_columns_in_order <- cols_in_order$columnames[which(cols_in_order$result_df_row < which_R &
...
#                         max(temp_mat3[genes_in_order$gene_name,cols_in_order$columnames ])),
#                       c("cyan4","cyan","lightcyan","white","pink", "red3", "darkred","black"))
# png(filename=paste0(dirs$figures,"full__heatmap_ontologic_ordering_zscores__muscle.png"),
#     height = 7, width=7, res = 250,unit="in")
# temp_mat4 <- temp_mat3[subset_genes_in_order,subset_columns_in_order]
# colnames(temp_mat4) <- make.unique(str_replace(colnames(temp_mat4),"\\.\\.\\.","."))
# colnames(temp_mat4) <- make.unique(str_remove(colnames(temp_mat4),"\\.\\..*"))
# Heatmap(temp_mat4,
...
# }
# rha <- rowAnnotation(foo = anno_block(
#   align_to = list("A"=3:100,B=320:450),
#   panel_fun = panel_fun,
#   width = unit(3,"cm")
# ))
# cha <- columnAnnotation(foo = anno_block(
#   align_to = list("A"=3:40,B=130:180),
#   panel_fun = panel_fun,
#   width = unit(3,"cm")
# ))
# 
# 
# Heatmap(temp_mat3[genes_in_order$gene_name,cols_in_order$columnames ],
#         cluster_rows = F,
#         cluster_columns = F,
#         col=col_fun,
#         left_annotation = rha,
...
# panel_fun <- function(index, nm){
#   grid.rect()
#   grid.text(paste0(length(index),0.5,0.5))
# }
...
temp_list <- c(gene_sets$spec_onto,list(ubiq_chrom=gene_sets$ubiq_chrom))
...
for(i in 1:length(unique(temp_df$chrom_subset_name))){
  ...
  plot_list[[i]] <- upset(fromExpression(temp_upvec), 
        nintersects = 40, 
...
for(i in 1:length(plot_list)){print(plot_list[[i]]);print(unique(temp_df$chrom_subset_name)[i])}
...
df <- data.frame(phen=ontos$hpo$enrichments__non_ubiq$name,num_ubiq=ontos$hpo$enrichments__ubiq$num_chrom_associated,num_nonubiq=ontos$hpo$enrichments__non_ubiq$num_chrom_associated, or_ubiq=ontos$hpo$enrichments__ubiq$odds_ratio, or_nonubiq=ontos$hpo$enrichments__non_ubiq$odds_ratio,p_ubiq=ontos$hpo$enrichments__ubiq$p.value,p_nonubiq=ontos$hpo$enrichments__non_ubiq$p.value)
