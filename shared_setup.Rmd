...
req_packages <- list(
  standard = c("BiocManager","doParallel","ggplot2","stringr", "reshape2", "XML",
               "ggiraph","ggraph","igraph","collapsibleTree","webshot","remotes","devtools",
               "dendextend","factoextra","cowplot","httr","jsonlite", "circlize","UpSetR","tidyverse"),
  biocmanager = c("Gviz","biomaRt","ComplexHeatmap","DOSE","pathview","clusterProfiler",
                  "org.Hs.eg.db","topGO","enrichplot","cmapR")
)
...
add_ancestors <- function(onto, ascendant_relations=c(), ascend_through_parents=TRUE){
  onto$ancestors <- vector("list",length=length(onto$id))
  add_ancestors_helper <- function(id, onto, ascendant_relations, ascend_through_parents,visited_ids=c()){
    which <- which(onto$id==id)
    if(length(which)==0){stop(paste("id", id, "does not exist in ontology"))}
    if(length(which)>1){stop(paste("id", id, "ambiguous in ontology"))}
    visited_ids <- c(visited_ids, id)
    relation_df <- onto$relations[[which]]
    all_transitive_ids <- relation_df$id[which(is.element(relation_df$relation, ascendant_relations))]
    if(ascend_through_parents){
      all_transitive_ids <- unique(c(all_transitive_ids,onto$parents[[which]]))
    }
    if(length(all_transitive_ids) > 0){
      onto$ancestors[[which]] <- unique(c(onto$ancestors[[which]],all_transitive_ids))
    }
    transitive_ids <- all_transitive_ids[!is.element(all_transitive_ids, visited_ids)]
    if(length(transitive_ids) == 0){
      if(length(onto$ancestors[[which]]==0)){
        onto$ancestors[[which]] <- NA 
      }
      return(onto)
    }
    transitive_ids_unassigned <- onto$id[which( sapply(onto$ancestors,length) == 0 & is.element(onto$id, transitive_ids))]
    for(tid in transitive_ids_unassigned){
     # print(tid)
      visited_ids <- c(visited_ids, tid)
      onto <- add_ancestors_helper(tid, onto, ascendant_relations, ascend_through_parents, visited_ids)
    }
    for(tid in all_transitive_ids){
      onto$ancestors[[which]] <- unique(c(onto$ancestors[[which]],
                                            onto$ancestors[[which(onto$id == tid)]]))
    }
    return(onto)
  }
  ...
  for(i in 1:length(onto$id)){
    if(i %% 1000 == 0){print(i)}
    if(length(onto$ancestors[[i]]) > 0){next}
    onto <- add_ancestors_helper(onto$id[i], onto,
...
while(TRUE){
  ...
  if(length(line)==0){
    ...
    ontos$go$ont$relations[term_num][[1]] <- rbind(ontos$go$ont$relations[term_num][[1]],
                                                       temp_df) 
...
ontos$go$ont <- add_inverse_relation(ontos$go$ont, "part_of",
                                         inverse_relation_name = "has_part(SUPP_INV(part_of))")
ontos$go$ont <- add_descendants(ontos$go$ont, descendant_relations = c("has_part(SUPP_INV(part_of))"))
save.image()
...
ontos$hpo$ont <- add_ancestors(ontos$hpo$ont)
...
##### add: UBERON:0018254 part_of UBERON:0002204 ## skeletal musculature part_of musculoskeletal system
##### add: UBERON: is_a UBERON: ## heart is_a muscle organ
...
ontos$uberon$ont$relations[[which(ontos$uberon$ont$name=="skeletal musculature")]] <- unique(rbind(
  ontos$uberon$ont$relations[[which(ontos$uberon$ont$name=="skeletal musculature")]],
  data.frame(relation="part_of",id=ontos$uberon$ont$id[which(ontos$uberon$ont$name=="musculoskeletal system")])
))

which_child <- which(ontos$uberon$ont$name=="heart")
which_parent <- which(ontos$uberon$ont$name=="muscle organ")
ontos$uberon$ont$children[[which_parent]] <- c(ontos$uberon$ont$children[[which_parent]],
                                               ontos$uberon$ont$id[[which_child]])  
ontos$uberon$ont$parents[[which_child]] <- c(ontos$uberon$ont$parents[[which_child]],
                                               ontos$uberon$ont$id[[which_parent]])


rm(which, which_keep, id, which_parent, which_child)
#################
...
ontos$uberon$ont <- add_inverse_relation(ontos$uberon$ont, "part_of",
                                         inverse_relation_name = "has_part(SUPP_INV(part_of))")
# ontos$uberon$ont <- add_inverse_relation(ontos$uberon$ont, "has_part",
#                                          inverse_relation_name = "part_of(SUPP_INV(has_part))")
ontos$uberon$ont <- add_depth(ontos$uberon$ont, ascendent_relations = c("part_of","part_of(SUPP_INV(has_part))"))
ontos$uberon$ont <- add_height(ontos$uberon$ont, descendant_relations = c("has_part(SUPP_INV(part_of))","has_part"))
#ontos$uberon$ont <- add_descendants(ontos$uberon$ont, descendant_relations = c("has_part(SUPP_INV(part_of))","has_part"))
## NOTE: "has_part some" relation causes issue with e.g. 
ontos$uberon$ont <- add_descendants(ontos$uberon$ont, descendant_relations = c("has_part(SUPP_INV(part_of))"))

#ontos$uberon$ont <- add_ancestors(ontos$uberon$ont, ascendant_relations = c("part_of","part_of(SUPP_INV(has_part))" ))
ontos$uberon$ont <- add_ancestors(ontos$uberon$ont, ascendant_relations = c("part_of" ))
...
ontos$cl$ont <- add_height(ontos$cl$ont)
##### done: read in and set up ontology ####
...
temp$non_syndrome_flag <- "" == str_remove(str_remove(temp$Phenotype,"\\{.*\\}\\(*[0-9]*\\)*"),"^ ")
temp$non_syndrome_flag <- temp$non_syndrome_flag | grepl("somatic",temp$Phenotype)
...
temp2 <- onto_maps$OMIM_GENE$map[c("Ensembl_Gene_ID","OMIM_ID","non_syndrome_flag")]
...
onto_maps$HPO_UBERON$map$temp_hpo_height <- ontos$hpo$ont$height[match(onto_maps$HPO_UBERON$map$HPO_ID, ontos$hpo$ont$id )]

### NOTE: FIX HPO UBERON MAPPING: 
##### add: HP:0033127 "abnormality of the musculoskeletal system" maps to UBERON:0002204 "musculoskeletal systems" 
onto_maps$HPO_UBERON$map$UBERON_ID[which(onto_maps$HPO_UBERON$map$HPO_ID=="HP:0033127")] <- "UBERON:0002204"


## 
### fill in missing uberon terms from map - find deepest HPO term with some UBERON mapped to it
#### need to start at deepest level and move upwards
temp_order <- order(onto_maps$HPO_UBERON$map$temp_hpo_height, decreasing = F)
for(i in temp_order){
  if(i %% 1e3 == 0){print(i)}
...
rm(temp, temp_df, ancestors, i, which, temp_order)
onto_maps$HPO_UBERON$map$temp_hpo_height <- NULL
...
# gene_sets$genes_all_go <- read.table( paste0(dirs$temp_data,"goa_human.gaf.gz"), skip=0, header = FALSE, sep = "\t",comment.char = "!",fill = T)
# gene_sets$paralogs <- NULL
...
#                               values=list(chr,"protein_coding"),
#                               mart=ensembl,useCache = F)
#   if(is.null(gene_sets$paralogs)){
#     gene_sets$paralogs <- temp
#   } else{
#     gene_sets$paralogs <- rbind(gene_sets$paralogs, temp)
#   }
# 
# }
for(chr in c(as.character(1:22),"X","Y","MT" )){
  print(chr)
  temp <- getBM(attributes=c('ensembl_gene_id',"external_gene_name",
                             "ensembl_transcript_id","transcript_length","cds_length",
...
gene_sets$genes_all_transcript_w_noncanon <- gene_sets$genes_all_transcript
...
gene_sets$human_chromatin_modifiers$go_terms = c(
...
for(i in 1:length(gene_sets$human_chromatin_modifiers$go_terms)){
  ...
  descendants <- c(term_id, ontos$go$ont$descendants[[which]])
...
# gene_sets$human_chromatin_modifiers$gene_subsets$non_histones <- setdiff(
#   gene_sets$human_chromatin_modifiers$gene_subsets$all_chrom_modifiers,
#   gene_sets$human_chromatin_modifiers$gene_subsets$`structural constituent of chromatin`)

gene_sets$human_chromatin_modifiers$AN_curated_set <- read.csv("./AN_curated_chromopathy_LB_mod.csv")

gene_sets$human_chromatin_modifiers$x__gene_subsets <- gene_sets$human_chromatin_modifiers$gene_subsets
for(i in 1:length(gene_sets$human_chromatin_modifiers$gene_subsets)){
  gene_sets$human_chromatin_modifiers$gene_subsets[[i]] <-intersect(gene_sets$human_chromatin_modifiers$gene_subsets[[i]],
                                                                    gene_sets$human_chromatin_modifiers$AN_curated_set$ensembl_gene_id) 
}
gene_sets$human_chromatin_modifiers$gene_subsets$all_chrom_modifiers <-
  unique(gene_sets$human_chromatin_modifiers$AN_curated_set$ensembl_gene_id) 

for(i in 1:length(gene_sets$human_chromatin_modifiers$gene_subsets)){print(paste0(names(gene_sets$human_chromatin_modifiers$gene_subsets)[i],"::", length(gene_sets$human_chromatin_modifiers$x__gene_subsets[[i]]) - length(gene_sets$human_chromatin_modifiers$gene_subsets[[i]])) )}
print("")
for(i in 1:length(gene_sets$human_chromatin_modifiers$gene_subsets)){print(paste0(names(gene_sets$human_chromatin_modifiers$gene_subsets)[i],"::", length(gene_sets$human_chromatin_modifiers$gene_subsets[[i]])) )}
## human
## mouse
## zebrafish
## celegans
...
### Add control gene sets
gene_sets$human_chromatin_modifiers$gene_subsets$control_set__non_chrom_mod <-
  setdiff(  gene_sets$genes_all$ensembl_gene_id[which(gene_sets$genes_all$gene_biotype=="protein_coding")] ,
  gene_sets$human_chromatin_modifiers$gene_subsets$all_chrom_modifiers)
...
gene_sets$lof_metrics <- lof_metrics[,c("gene", "gene_id","pLI","pRec","pNull")]
rm(lof_metrics)
save.image()
```

# ```{r, download taxonomy info}
# files$taxonomy <- paste0(dirs$temp_data,"taxmp.zip")
# download.file("https://ftp.ncbi.nih.gov/pub/taxonomy/taxdmp.zip",files$taxonomy)
# dirs$taxonomy <- paste0(dirs$temp_data,"taxonomy/")
# dir.create(dirs$taxonomy)
# system(paste0("unzip -o ", files$taxonomy," -d ",dirs$taxonomy))
# 
# taxonomy <- list()
# taxonomy$names <- read.csv(paste0(dirs$taxonomy,"names.dmp"), sep="\t", header = F)
# taxonomy$names <- taxonomy$names[,which(!apply(taxonomy$names,2, function(x){all(x=="|")}))]
# colnames(taxonomy$names)[1:4] <- c("tax_id","name_txt","unique name", "name class")
# taxonomy$nodes <- read.csv(paste0(dirs$taxonomy,"nodes.dmp"), sep="\t", header = F)
# taxonomy$nodes <- taxonomy$nodes[,which(!apply(taxonomy$nodes,2, function(x){all(x=="|")}))]
# colnames(taxonomy$nodes)[1:2] <- c("tax_id","parent tax_id")
# 
# h_sapiens_lineage <- data.frame(id=character(),name=character(), height=integer())
# next_id <- taxonomy$names$tax_id[which(taxonomy$names$name_txt=="Homo sapiens")]
# root_id <- taxonomy$names$tax_id[which(taxonomy$names$name_txt=="root")]
# height <- 1
# while(next_id != root_id){
#   temp_name <- taxonomy$names$name_txt[which(taxonomy$names$tax_id==next_id)]
#   if(length(temp_name)==0){temp_name <- NA}
#   temp_df <- data.frame(id=next_id, name=temp_name, height=height)
#   h_sapiens_lineage <- rbind(h_sapiens_lineage,temp_df)
#   next_id <- taxonomy$nodes$`parent tax_id`[which(taxonomy$nodes$tax_id==next_id)]
#   height=height+1
# }
# h_sapiens_lineage$depth <- max(h_sapiens_lineage$height) - h_sapiens_lineage$height
# taxonomy$human_lineage <- h_sapiens_lineage
# taxonomy$names <- taxonomy$nodes <- NULL
# ```
```{r, uniprot domains}

temp_map <- getBM(attributes=c('ensembl_gene_id',"external_gene_name","uniprotswissprot","uniprotsptrembl"), mart = ensembl, useCache = F)
temp <- tempfile()
#download.file("https://rest.uniprot.org/uniprotkb/stream?fields=accession%2Creviewed%2Cid%2Cgene_names%2Corganism_name%2Clength%2Cprotein_families%2Cft_region%2Cft_domain&format=tsv&query=%28*%29+AND+%28model_organism%3A9606%29+AND+%28reviewed%3Atrue%29",temp)
download.file("https://rest.uniprot.org/uniprotkb/stream?fields=accession%2Creviewed%2Cid%2Cgene_names%2Corganism_name%2Clength%2Cprotein_families%2Cft_region%2Cft_domain&format=tsv&query=%28*%29+AND+%28model_organism%3A9606%29+AND+%28reviewed%3Atrue%29",temp)
protein_domains <- read.table( temp, skip=0, header = TRUE, sep = "\t", fill=TRUE, quote = "")
protein_domains$prop_disordered <- NA
protein_domains$disorder_starts <- NA
protein_domains$disorder_ends <- NA
protein_domains$disorder_lengths <- NA
for(i in 1:nrow(protein_domains)){
  temp <- strsplit(protein_domains$Region[i],"(?<=.)(?=REGION)",perl = T)[[1]]
  temp_df <- data.frame(start=numeric(),
                        end=numeric(),
                        type=character())
  for(j in 1:length(temp)){
    temp_df <- rbind(temp_df,
                     data.frame(
                       start=as.numeric(str_extract(str_extract(temp[j],"REGION [0-9]*"),"[0-9]+")),
                       end=as.numeric(str_remove(str_extract(temp[j],"REGION.*\\.\\.[0-9]*"),".*\\.\\.")),
                       type=str_remove(str_extract(temp[j],"note=\\\"[a-zA-Z0-9 \t]*"),"note=\\\"")
                     )) 
  }
  which <- which(temp_df$type == "Disordered")
  protein_domains$prop_disordered[i] <- sum(temp_df$end[which] - temp_df$start[which]) / protein_domains$Length[i]
  if(length(which)==0){next}
  protein_domains$disorder_starts[i] <- paste(temp_df$start[which],collapse=",")
  protein_domains$disorder_ends[i]  <- paste(temp_df$end[which],collapse=",")
  protein_domains$disorder_lengths[i]  <- paste(temp_df$end[which]-temp_df$start[which],collapse=",")
}

protein_domains$ensembl_gene_id <- NA
protein_domains$ensembl_gene_id <- temp_map$ensembl_gene_id[match(protein_domains$Entry,temp_map$uniprotswissprot)]
which <- which(is.na(protein_domains$ensembl_gene_id))
protein_domains$ensembl_gene_id[which] <- temp_map$ensembl_gene_id[match(protein_domains$Entry[which],temp_map$uniprotsptrembl)]
which <- which(is.na(protein_domains$ensembl_gene_id))
for(i in 1:length(which)){
  temp <- gene_sets$genes_all$ensembl_gene_id[is.element(gene_sets$genes_all$external_gene_name, str_split(protein_domains$Gene.Names[which[i]]," ")[[1]])]
  protein_domains$ensembl_gene_id[which[i]] <- ifelse(length(temp)==1,temp,NA)
}
```
