...
i=1
while(file.size(paste0(temp_dir,"gene_count.txt")) > 0){
  print(i)
  if(i > temp_num){stop("stop: exceeded expected chunk count")}
  system(paste0("head -n ", format(temp_nrow, scientific = F), " ",
                temp_dir,"gene_count.txt",
                " > ", temp_dir,"temp_split/chunk_",i, 
                "; sed -i -e '1,", format(temp_nrow, scientific = F), "d' ",
                temp_dir,"gene_count.txt"
                )
  )
  i <- i+1
...
# system(paste0("split ",temp_dir,"GSE119945_gene_count.txt",
#               " -n ", "l/",temp_num, " ",
