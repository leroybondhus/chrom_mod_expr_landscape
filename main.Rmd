...
for(i in 1:length(gene_sets$human_chromatin_modifiers$gene_subsets)){
  ...
  ggl[[length(ggl)+1]] <- ggplot(df, aes(log10(gene_length)))+
    geom_histogram(bins=200,  fill="grey20")+
    coord_cartesian(ylim = c(0.2,0.5), xlim = c(2, 6.5))+
    barcode_theme
  names(ggl)[length(ggl)] <- paste0(chrom_subset_name, ": ", "gene_length")
  
  ## transcript length barcode plot
  ...
  ggl[[length(ggl)+1]] <- ggplot(df, aes(log10(transcript_length)))+
    geom_histogram(bins=200, fill="grey20")+
    coord_cartesian(ylim = c(0.2,0.5), xlim = c(2, 6.5))+
    barcode_theme
  names(ggl)[length(ggl)] <- paste0(chrom_subset_name, ": ", "transcript_length")
  ...
  ggl[[length(ggl)+1]] <- ggplot(df, aes(x=log10(exon_length),y=gene_factor_num))+
    geom_point(fill="grey20") +
    coord_cartesian( xlim = c(1, 4))+
    barcode_theme
  names(ggl)[length(ggl)] <- paste0(chrom_subset_name, ": ", "rand_exon_length")
  
  gene_sets$genes_all_transcript
...
ggplot(df, aes(log10(transcript_length, )))+
