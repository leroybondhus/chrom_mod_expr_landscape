...
get_descendants_plus <- function(id, onto,
                                 descendant_relations=c(),
  ...
  descendants <- id
  visited_ids <- c(visited_ids, transitive_ids)
  for(tid in transitive_ids){
    descendants <- unique(c(descendants,
                            get_descendants_plus(tid, onto,
...
## may need dynamic programming speed up here... 
# get_height_plus <- function(id, onto,
#                             descendant_relations=c(),
#                             descend_through_children=TRUE,
#                             visited_ids=c()){
#   which <- which(onto$id==id)
...
add_height <- function(onto, descendant_relations=c(), descend_through_children=TRUE){
  ...
  add_height_helper <- function(id, onto, descendant_relations, descend_through_children,visited_ids=c()){
    ...
    for(tid in transitive_ids_unassigned){
      visited_ids <- c(visited_ids, tid)
      onto <- add_height_helper(tid, onto, descendant_relations, descend_through_children, visited_ids)
    }
    onto$height[which] <- max(onto$height[which(is.element(onto$id, transitive_ids))])+1
    return(onto)
  }
...
## may need dynamic programming speed up here... 
# get_depth_plus <- function(id, onto,
#                       ascendant_relations=c(),
...
#   }
#   relation_df <- onto$relations[[which]]
#   transitive_ids <- relation_df$id[which(is.element(relation_df$relation, ascendant_relations))]
...
add_depth <- function(onto, ascendant_relations=c(), ascend_through_parents=TRUE, min_or_max=function(x){min(x)} ){
  onto$depth <- numeric(length=length(onto$id))
  
  add_depth_helper <- function(id, onto, ascendant_relations, ascend_through_parents, visited_ids=c()){
    which <- which(onto$id==id)
    if(length(which)==0){stop(paste("id", id, "does not exist in ontology"))}
    if(length(which)>1){stop(paste("id", id, "ambiguous in ontology"))}
    ...
    ## below line should no longer be needed since all visited ids should now be filtered out by "onto$depth == 0" in above line 
    #transitive_ids_unassigned <- transitive_ids_unassigned[!is.element(transitive_ids_unassigned, visited_ids)]
    
    
    while(length(transitive_ids_unassigned) > 0 ){
      tid  <- transitive_ids_unassigned[1]
      onto <- add_depth_helper(tid, onto, ascendant_relations, ascend_through_parents,
                               visited_ids=c(visited_ids, id, tid))
      transitive_ids_unassigned <- onto$id[which(onto$depth == 0 & is.element(onto$id, transitive_ids))]
      transitive_ids_unassigned <- transitive_ids_unassigned[!is.element(transitive_ids_unassigned, visited_ids)]
    }
    ### issue that may still be some ids unassigned but that were "visited" earlier (i.e. gray nodes)
    ### can ignore these for assignment since they should always lose or technically be infinite 
  ...
  while(length(which(is.infinite(onto$depth))) > 0){
    print(paste("fixing prelim inf depth assignments: iteration", inf_check_num))
...
onto <- add_depth(onto, ascendant_relations = c("has_part(SUPP_INVERSE)"))
...
ontos$hpo$ont$height <- numeric(length=length(ontos$hpo$ont$id))
ontos$hpo$ont$depth <- numeric(length=length(ontos$hpo$ont$id))
for(i in 1:length(ontos$hpo$ont$id)){
  if(i %% 500 == 0){print(i)}
  ontos$hpo$ont$height[i] <- get_height(ontos$hpo$ont$id[i],ontos$hpo$ont)
  ontos$hpo$ont$depth[i] <- get_depth(ontos$hpo$ont$id[i],ontos$hpo$ont)
}
...
ontos$uberon$ont <- add_inverse_relation(ontos$uberon$ont, "part_of",
                                         inverse_relation_name = "has_part(SUPP_INVERSE)")
##### need to add inversion of "part_of" relationship to uberon - else undercounting heart

### NOTE: UBERON IMPROPER ANNOTATIONS: 
## This might be error of my interpretation of "has_part" ##### remove: UBERON:0005984 has_part UBERON:0001021: subendocardium layer has_part nerve 
...
# which <- which(ontos$uberon$ont$id=="UBERON:0005984")
# which_keep <- which(!(grepl("UBERON:0001021",ontos$uberon$ont$relations[[which]]$id) &
...
```{r, experimental ontology layered analysis, hpo}
...
ids <- get_descendants(onto$id[which(onto$name=="Abnormal heart morphology")] , onto)
ids <- get_descendants(onto$id[which(onto$name=="Abnormal hair morphology")] , onto)
...
which <- which(temp_vertices$depth <= 6 )
...
ggraph(temp_graph, layout = "circlepack",weight=size)+
  geom_node_circle(aes(fill=signif_val))+
  geom_node_label(aes(label=label,fill=signif_val), size=2)+#, position = position_dodge())+
...
ggraph(temp_graph, layout = "circlepack",weight=size)+
  geom_node_(aes(fill=signif_val))+
  geom_node_label(aes(label=label,fill=signif_val), size=2)+#, position = position_dodge())+
  scale_fill_gradientn(colours = c("grey40","#ffaaaa", "red","yellow"),
                       breaks = c(0,1.3, 2, 3, max(temp_vertices$signif_val) ),
                       guide="colorbar"
...
onto <- add_height(onto, descendant_relations = c("is_a","part_of"))
### replace depth with add_depth function..... oi__io
options(expressions=30000)
onto <- add_depth(onto, ascendant_relations = c("has_part(SUPP_INVERSE)"))
## NEED TO GENERALIZE GET HEIGHT AND GET DEPTH TO ACCOMODATE IS_A AND HAS_PART RELATIONS

for(i in 1:length(onto$id)){
  if(i %% 500 == 0){print(i)}
  #onto$height[i] <- get_height_plus(onto$id[i],onto, descendant_relations = c("is_a","part_of"))
  onto$depth[i] <- get_depth_plus(onto$id[i],onto, ascendant_relations = c("has_part(SUPP_INVERSE)"))
}
...
for( layer in 1:max(onto$height)){
  ...
  for( el in els_in_layer ){
    if(i %% 250 == 0){print(i)}
    i <- i+1
    ubs <- get_descendants_plus(onto$id[el], onto,
                                descendant_relations = c("has_part(SUPP_INVERSE)"))
...
ascendant_relations = c("part_of")
for(i in 1:length(onto$id)){
  if(i %% 2e3 == 1){print(i)}
  rids <- onto$relations[[i]]$id[which(is.element(onto$relations[[i]]$relation, ascendant_relations))]
  rs <- onto$relations[[i]]$relation[which(is.element(onto$relations[[i]]$relation, ascendant_relations))]
...
## select subset of hpo ids to plot
ids <- onto$descendants[[which(onto$name=="heart")]]
...
ids <- onto$descendants[[which(onto$name=="skin epidermis")]]
...
temp_vertices <- data.frame(name=ids, size=(temp_df$num_chrom_associated[match(ids,temp_df$id)]+1)*10,
                            ...
                            depth=temp_df$depth[match(ids,temp_df$id)])
temp_vertices$label <- NA
which <- which(temp_vertices$depth <= 9 )
temp_vertices$label[which] <- onto$name[match(temp_vertices$name[which], onto$id)]
