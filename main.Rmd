...
ontos$uberon$ont <- add_depth(onto, ascendant_relations = c("part_of"))
...
ontos$uberon$ont <- add_depth(ontos$uberon$ont, ascendant_relations = c("part_of"))
...
which <- which(onto$name=="Abnormal heart morphology")
ids <- c(onto$id[which], onto$descendants[[which]])
which <- which((onto$name=="Abnormal hair morphology"))
ids <- c(onto$id[which], onto$descendants[[which]])
...
ggraph(temp_graph, layout = "circlepack",weight=size)+
  ...
  geom_node_label(aes(label=label,fill=signif_val), size=2,repel = TRUE) +# position = position_dodge())+
...
ascendant_relations = c("has_part(SUPP_INV(part_of))")
for(i in 1:length(onto$id)){
  if(i %% 2e3 == 1){print(i)}
...
temp_vertices <- data.frame(name=ids, size=(temp_df$num_chrom_associated[match(ids,temp_df$id)]+1)*10,
                            ...
                            height=temp_df$height[match(ids,temp_df$id)])
temp_vertices$label <- NA
which <- which(temp_vertices$depth <= 10 )
temp_vertices$label[which] <- onto$name[match(temp_vertices$name[which], onto$id)]
