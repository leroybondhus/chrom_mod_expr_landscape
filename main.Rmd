  ...
  if(length( onto$children[[which]]) > 0 ){
    for(child in onto$children[[which]]){
      temp_df <- rbind(temp_df,
                       get_desc_summary_table(child, onto, level=level+1))
    }
  }
...
heart_chamber_df <- get_desc_summary_table("UBERON:0004151", temp_onto)
heart_chamber_df$num_datasets <- sapply(str_split(heart_chamber_df$data_cols,","), function(x){ifelse(all(x==""),0,length(x)) })
heart_chamber_df_sub <- heart_chamber_df[which(heart_chamber_df$data_cols != ""),c("id","name","num_datasets","level")]
heart_chamber_df_sub <- heart_chamber_df_sub[order(heart_chamber_df_sub$level),]

write.csv(muscle_df_sub, paste0(dirs$tables,"ENCODE_mesoderm_derived_RNAseq.csv"))
write.csv(heart_chamber_df_sub, paste0(dirs$tables,"ENCODE_heart_chamber_RNAseq.csv"))
