...
```{r load libraries}
req_packages <- list(
  standard = c("BiocManager","doParallel","ggplot2","stringr", "reshape2"),
...
for(i in 1:length(gene_sets$human_chromatin_modifiers$gene_subsets)){
  ...
  temp_name <- paste0(chrom_subset_name, ": ", "gene_length")
  ggl[[temp_name]] <- ggplot(df, aes(log10(gene_length)))+
  ...
  temp_name <- paste0(chrom_subset_name, ": ", "transcript_length")
  ggl[[temp_name]] <- ggplot(df, aes(log10(transcript_length)))+
    geom_histogram(bins=200, fill="grey20")+
    coord_cartesian(ylim = c(0.2,0.5), xlim = c(2, 6.5))+
    barcode_theme
  ...
  temp <- temp[order(temp$L1, temp$value),]
  temp$L1 <- as.factor(temp$L1)
  
  
  ggplot(temp, aes(log10(value), percentile, group=L1, color=L1))+
    geom_line()
