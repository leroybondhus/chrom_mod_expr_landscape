...
colnames(dbs$MIM_gene_to_disease$map) <- str_replace_all(temp_header,"[[:space:]]","_")
...
dbs$MIM_gene_to_disease$map <- dbs$MIM_gene_to_disease$map[,c("Ensembl_Gene_ID","Phenotypes")]

temp <- str_split(dbs$MIM_gene_to_disease$map$Phenotypes,";")
names(temp) <- dbs$MIM_gene_to_disease$map$Ensembl_Gene_ID
...
dbs$MIM_gene_to_disease$map <- temp

dbs$HPO_gene_phen <- list()
##### try other file......
dbs$HPO_gene_phen$URL <- "http://purl.obolibrary.org/obo/hp/hpoa/phenotype.hpoa"
files$HPO_gene_phen <- paste0(dirs$temp_data,"phenotype.hpoa")
...
dbs$HPO_gene_phen$map <- read.csv(files$HPO_gene_phen,sep="\t",skip=4)
dbs$HPO_gene_phen$map <- dbs$HPO_gene_phen$map[grepl("OMIM",dbs$HPO_gene_phen$map$X.DatabaseID),]
colnames(dbs$HPO_gene_phen$map)[which(colnames(dbs$HPO_gene_phen$map)=="X.DatabaseID")] <- "OMIM_ID"


dbs$HPO_OMIM_GENE <- list()
temp1 <- dbs$HPO_gene_phen$map[,c("OMIM_ID","HPO_ID")]
temp2 <- dbs$MIM_gene_to_disease$map[c("MIM_Number","Ensembl_Gene_ID")]
temp2$OMIM_ID <- paste0("OMIM:",temp2$MIM_Number)
temp2$MIM_Number <- NULL
dbs$HPO_OMIM_GENE$map <- merge(temp1,temp2, by="OMIM_ID")



### HPO to UBERON
## hpo
dbs$HPO_to_UBERON <- list()
dbs$HPO_to_UBERON$URL <- "https://github.com/obophenotype/human-phenotype-ontology/raw/master/hp.owl"

files$hp_owl <- paste0(dirs$temp_data,"hp.owl")
download.file(dbs$HPO_to_UBERON$URL,files$hp_owl)
...
while(TRUE){
  ...
  if(length(line)==0){
    ...
    term_num <- term_num + 1
    dbs$HPO_to_UBERON$map$HPO_id[term_num] <- str_extract(line, "HP_[0-9]+")
    on_HPO_term <- TRUE
  } else if(grepl("UBERON",line) & on_HPO_term){
    dbs$HPO_to_UBERON$map$UBERON_terms[[term_num]] <- c(dbs$HPO_to_UBERON$map$UBERON_terms[[term_num]], 
...
dbs$HPO_to_UBERON$map$UBERON_terms[sapply(dbs$HPO_to_UBERON$map$UBERON_terms, is.null)] <- NA 
names(dbs$HPO_to_UBERON$map$UBERON_terms) <- dbs$HPO_to_UBERON$map$HPO_id
temp <- reshape2::melt(dbs$HPO_to_UBERON$map$UBERON_terms)
colnames(temp) <- c("UBERON_ID","HPO_ID")
temp$UBERON_ID <- str_replace_all(temp$UBERON_ID, "_",":")
temp$HPO_ID <- str_replace_all(temp$HPO_ID,"_",":")
temp <- temp[,c("HPO_ID","UBERON_ID")]
...
for(i in 1:length(ontos$hpo$ont$id)){
  ...
  fisher.test(x=cbind(c(),
                      c()))
  
}


## plot Gene-Gene phenotype similarity (assymetric if want subsumption test e.g.  if A has subset of phens of B)
