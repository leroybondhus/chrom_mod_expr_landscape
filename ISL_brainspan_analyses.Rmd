...
brainspan_terms <- Filter(Negate(is.null), onto$brainspan_cols)
 # 130 Uberon terms that relate to brainspan data

# histogram to show avg number of terms per brainspan column
brainspan_term_counts <- vector("integer", length(brainspan_terms))
for(i in 1:length(brainspan_terms)){
  brainspan_term_counts[i] = length(brainspan_terms[[i]])
}

hist(brainspan_term_counts, breaks = 20, main = "Histogram of number of brainspan terms populated")
