...
columns_meta$age <- convert_age(columns_meta$age)

# sort columns_meta by brainspan structure name and then age ascending order
sorted_cols <- columns_meta[with(columns_meta, order(structure_name, age)),]
...
sorted_expression <- data.matrix(sorted_expression) # convert to matrix
```
...
heatmap(sorted_expression[1:1500,1:33], labRow = rows_meta$gene_symbol[1:1000], labCol = sorted_cols$structure_name)
```
...
corr_matrix <- matrix(data = NA, nrow=nrow(expression_mat), ncol=nrow(struct_to_ids))
p_val_matrix <- matrix(data = NA, nrow=nrow(expression_mat), ncol=nrow(struct_to_ids))
confidence_int_lower_matrix <- matrix(data = NA, nrow=nrow(expression_mat), ncol=nrow(struct_to_ids))
confidence_int_upper_matrix <- matrix(data = NA, nrow=nrow(expression_mat), ncol=nrow(struct_to_ids))
...
for(i in 1:nrow(struct_to_ids)){ # for each unique brain structure
  ...
  temp_mat <- sorted_expression[,grep(struct, sorted_cols$structure_name, fixed = T)]
...
# using cor.test brainspan structures with only two samples also lead to error
...
chrom_mod_expression <- sorted_expression[matched_modifiers,]
which(chrom_mod_corr[,col_index]>0.8)
...
ggplot(data.frame(log10(temp_cols$age), temp_mat[which(chrom_mod_corr[,col_index]>0.8),]), aes(x = log10(temp_cols$age), y = temp_mat[which(chrom_mod_corr[,col_index]>0.8),])) + geom_line() + labs(x = "log10(age)", y = "expression level", title =paste0(interest_gene, " in ", struct, " over time"))
...
```{r, KAT6A/B expression plots}
# KAT6A, KAT6B, ING5, BRPF1/2/3, (H)MEAF6, FOXG1
kat6a_row <- which(rows_meta[matched_modifiers,]$gene_symbol == "KAT6A")
kat6b_row <- which(rows_meta[matched_modifiers,]$gene_symbol == "KAT6B")

# normalize gene expression btwn 0-1 - do i scale along row (gene) or column (sample)
norm_chrom_mod_expression <- chrom_mod_expression

kat6a_expression <- norm_chrom_mod_expression[kat6a_row,]
kat6b_expression <- norm_chrom_mod_expression[kat6b_row,]
pcw <- sorted_cols$age
structures <- sorted_cols$structure_name

kat6a_df <- data.frame(structures, pcw, kat6a_expression)
ggplot(kat6a_df, aes(x = log10(pcw), y = log10(kat6a_expression+1), color = structures, group = structures)) + geom_line() + geom_point() + facet_wrap(structures~.)+
  theme(legend.position = "none") + ggtitle("KAT6A expression over time for each brain region")

kat6b_df <- data.frame(structures, pcw, kat6b_expression)
ggplot(kat6b_df, aes(x = log10(pcw), y = log10(kat6b_expression+1), color = structures, group = structures)) + geom_line() + geom_point() + facet_wrap(structures~.)+
  theme(legend.position = "none")+ ggtitle("KAT6B expression over time for each brain region") + geom_vline(xintercept = log10(40)) + geom_vline(xintercept = log10(52*15))+ geom_vline(xintercept = log10(52*10))
