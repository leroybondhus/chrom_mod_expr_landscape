...
dirs <- list(data="./data/",
             ...
             cluster_analysis = "./results/figures/cluster_analysis/")
...
figure_dirs <- paste0(dirs$cluster_analysis, str_replace_all(struct_to_ids$unique_structures, "[ /]", "_"), "/")
...
```{r, correlation clustering}
struct = "dorsolateral prefrontal cortex"
struct_index = grep(struct, sorted_cols$structure_name, fixed = T)
struct = str_replace_all("dorsolateral prefrontal cortex", "[ /]", "_") 

# get matrix of correlation btwn pairs of genes
# use 0.5 as cutoff for difference between genes
# first subset expr matrix to get 1 structure (amygdaloid complex) & then filter chromatin modifier genes with a significant difference in expression level
struct_matrix <- normal_expr[matched_modifiers, struct_index] # 711 genes
sub_matrix <- struct_matrix[which(apply(struct_matrix, 1, max)-apply(struct_matrix, 1, min)>0.5),] # 498 genes

# get list of genes that filtered out b/c low variation in expression
cluster0 <- rows_meta[strtoi(row.names(struct_matrix[which(apply(struct_matrix, 1, max)-apply(struct_matrix, 1, min)<=0.5),])),]

write.csv(cluster0, paste0(dirs$tables, date, struct, "_cluster0.csv"))

# mean (normalized to [0-1])  expression per timepoint for each cluster 
mean_expr <- t(apply(sub_matrix, 1, rescale)) # normalizes by row (gene)

# create matrix of correlation btwn pairs of genes
gene_pairwise_corr <- -1*(cor(t(mean_expr))-1)
# gene_pairwise_corr <- cor(t(sub_matrix))
...
n_clusters_high <- max(myhcl[,2])
n_clusters_modest <- max(myhcl[,1])

# find subclusters for cutoff=0.4 clusters based on the cutoff=0.2 clusters
myhcl <- as.data.frame(myhcl)
colnames(myhcl)[1] = "modest"
colnames(myhcl)[2] = "high"
test <- vector("character", length(myhcl[,1]))
for (i in 1:n_clusters_high){
  subclusters <- unique(myhcl$modest[which(myhcl$high == i)])
  subcluster_indexes <- which(myhcl$high == i)
  n_sub_clusters <- length(unique(myhcl$modest[subcluster_indexes]))
  for (j in 1:n_sub_clusters){
    which(myhcl$modest[subcluster_indexes] == subclusters[j])
  }
...
# myhcl_mean_expr <- vector("list", n_clusters)
# for (i in 1:n_clusters){
#   # error in colMeans if only 1 gene in a cluster so check length
#   if(length(which(myhcl == i)) > 1){
#     myhcl_mean_expr[[i]] = colMeans(mean_expr[which(myhcl == i),])
#   }
#   else{
#     myhcl_mean_expr[[i]] = mean_expr[which(myhcl == i),]
#   }
# }
...
# myhcl_counts <- as.data.frame(table(myhcl))
# myhcl_df <- melt(myhcl_mean_expr)
# myhcl_df$pcw <- sorted_cols$age[struct_index]
# colnames(myhcl_df)[1] = "expr"
# colnames(myhcl_df)[2] = "cluster"
# myhcl_df$freq <- myhcl_counts$Freq[match(myhcl_df$cluster, myhcl_counts$myhcl)]
# myhcl_df$group_name <- paste0("cluster:",myhcl_df$cluster, ",num_genes:", myhcl_df$freq)


high_hcl_counts <- as.data.frame(table(myhcl[,2]))
...
# high_hcl_df$subcluster <- melt(modest_hcl_mean_expr)[,2]
...
modest_hcl_counts <- as.data.frame(table(modest_hcl))
...
modest_hcl_df$freq <- modest_hcl_counts$Freq[match(modest_hcl_df$cluster, modest_hcl_counts$modest_hcl)]
...
ggplot(select_high_hcl_df, aes(x = log10(pcw), y = expr)) +
      geom_point() + geom_line() +
      facet_wrap(~factor(group_name, levels=unique(select_high_hcl_df$group_name))) +
...
ggplot(select_modest_hcl_df, aes(x = log10(pcw), y = expr)) +
      geom_point() + geom_line() +
      facet_wrap(~factor(group_name, levels=unique(select_modest_hcl_df$group_name))) +
theme(legend.position = "none") +
      ggtitle(paste0(struct, " gene expression over time for each cluster (cutoff = 0.2)")) + 
      labs(x = "log10(pcw)", y = "normalized expr") + 
     geom_vline(xintercept = log10(40), color = "red") + geom_vline(xintercept = log10(52*15), color = "red") + geom_vline(xintercept = log10(52*10), color = "red")

# see what these coexpression patterns look like 
gg <- ggplot(myhcl_df, aes(x = log10(pcw), y = expr)) +
      geom_point() + geom_line() +
      facet_wrap(~factor(group_name, levels=unique(myhcl_df$group_name))) +
theme(legend.position = "none") +
      ggtitle(paste0(struct, " gene expression over time for each cluster")) + 
      labs(x = "log10(pcw)", y = "normalized expr") + 
     geom_vline(xintercept = log10(40), color = "red") + geom_vline(xintercept = log10(52*15), color = "red") + geom_vline(xintercept = log10(52*10), color = "red")

ggsave(paste0(dirs$cluster_analysis, struct, "/", struct, "_test_coexpression_over_time.png"), plot = gg, width = 10, height = 8)

write.csv(rows_meta[strtoi(row.names(myhcl[which(myhcl$high == 3),])),], paste0(dirs$tables,date,"_genes_in_cluster3_dorso_prefr_cortex(cutoff=0.4).csv"))

# create subdirectories in each structure results folder for plots for each cluster
cluster_dirs <- paste0("cluster", 1:n_clusters, "/")

for(dir in figure_dirs){
  for (sub_dir in cluster_dirs){
...
for(i in 1:n_pages(subplots)){
  ...
  ggsave(paste0(dirs$cluster_analysis, struct, "/", "cluster", i, "/", struct, "_cluster_", i, "_test_coexpression_over_time.png"), plot = p_save, width = 10, height = 8)
}


# make histogram of max expression for genes within a cluster
cluster_max_expr <- vector("list", max(n_clusters))
for (i in 1:n_clusters){
  # 
  if(length(which(myhcl == i)) > 1){
    cluster_max_expr[[i]] = apply(sub_matrix[which(myhcl == i),], 1, max, na.rm=TRUE)
    
  }
  else{
    cluster_max_expr[[i]] = max(sub_matrix[which(myhcl == i),])
  }
}
plot(cluster_max_expr[[2]])

# GO analysis (get code from aileen)
