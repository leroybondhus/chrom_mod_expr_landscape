...
sub_matrix <- struct_matrix[which(apply(struct_matrix, 1, max)-apply(struct_matrix, 1, min)>0.5),] # 498 genes (amygdaloid)
...
# cutoff height = 0.4
png(filename = paste0(dirs$cluster_analysis, struct, "/", struct, "_clustering_dendrogram(cutoff=0.4).png"), width = 960, height = 480)
plot(hc, main = "Cluster Dendrogram (cutoff = 0.4)")
rect.hclust(hc, h=0.4)
dev.off()

# cutoff height = 0.2
png(filename = paste0(dirs$cluster_analysis, struct, "/", struct, "_clustering_dendrogram(cutoff=0.2).png"), width = 960, height = 480)
...
n_clusters_high <- max(myhcl[,2]) # 51 (amyg)
n_clusters_modest <- max(myhcl[,1]) # 173 (amyg)
...
subcluster_names <- vector("character", length(myhcl[,1]))
for (i in 1:n_clusters_high){
  ...
  for (j in 1:n_sub_clusters){
    subcluster_names[which(myhcl$modest == subclusters[j])] = paste0(i,".",j)
  }
}
...
subcluster_hcl_counts <- as.data.frame(table(myhcl$subcluster))
...
modest_hcl_df$subcluster <- rep(unique(myhcl)$subcluster, each = length(struct_index))
modest_hcl_df$group_name <- paste0("subcluster:",modest_hcl_df$subcluster, ",num_genes:", modest_hcl_df$freq)
...
gg <- ggplot(select_modest_hcl_df, aes(x = log10(pcw), y = expr)) +
      geom_point() + geom_line() +
...
ggsave(paste0(dirs$cluster_analysis, struct, "/", struct, "_subcluster_expression_over_time(cutoff=0.2).png"), plot = gg, width = 10, height = 8)
...
# make histogram of max expression for genes within a cluster - WHICH CUTOFF TO USE
cluster_max_expr <- vector("list", max(n_clusters_high))
for (i in 1:n_clusters_high){
  # 
  if(length(which(myhcl$high == i)) > 1){
    cluster_max_expr[[i]] = apply(sub_matrix[which(myhcl$high == i),], 1, max, na.rm=TRUE)
    
  }
  else{
    cluster_max_expr[[i]] = max(sub_matrix[which(myhcl$high == i),])
  }
}
hist(cluster_max_expr[[1]], breaks = 50)

# GO analysis (get code from aileen)
# code to pull a list of genes in a given cluster (what cluster cutoff to use or use subclusters?)
rows_meta[strtoi(row.names(myhcl[which(myhcl$subcluster == "3.1"),])),]
