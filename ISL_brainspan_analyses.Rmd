...
struct_index = grep("amygdaloid complex", sorted_cols$structure_name, fixed = T)
...
for (i in 1:n_clusters){
  # error in colMeans if only 1 gene in a cluster
...
ggplot(test_df, aes(x = log10(pcw), y = expr, color = cluster, group = cluster)) +
...
for (i in 1:length(gene_list)){
  ...
  for (j in 1:length(partitioned_dfs)){
    ...
    ggsave(paste0("./results/figures/", date, "_gene_expr_plots/", time_partitions[j], '/', gene, "_", time_partitions[j], "_expression_plot.png"), plot = gg, width = 7, height = 5) 
  }
}
