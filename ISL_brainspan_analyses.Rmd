...
expression_mat <- read.csv(paste0(dirs$data, "brainspan/expression_matrix.csv"))
...
# note that onto <- ontos$uberon$ont directly from main or phenotype_summary_analyses
...
for (i in 1:length(columns_meta$structure_name)){
  mapped_uberon_ids[i] = struct_to_ids$mapped_ids [grep(columns_meta$structure_name[i],struct_to_ids$unique_structures, fixed = TRUE)]
}
...
for(i in 1:length(columns_meta$structure_name)){
  ...
  onto <- add_annotation(mapped_uberon_ids, onto, "brainspan_cols", i,
