...
# note that need to determine method to subset genes of interest b/c vector allocation issues
...
calc_correlation <- function(cols_meta,expr_mat){
  corr_vector <- vector("numeric", dim(expr_mat)[1])
  for(i in 1:dim(expr_mat)[1]){
    corr_vector[i] = cor(cols_meta$age, expr_mat[i,])
  }
  return(corr_vector)
}

calc_correlation(sorted_cols[1:33,], sorted_expression[,1:33])

# convert age data in columns_meta to be numeric
convert_age <- function(ages){
  numeric_ages <- vector("integer", length(ages))
  
  ages[ages == grepl("pcw", ages)]
  return(numeric_ages)
}
```
...
temp_els <- grepl("^hippocampal formation$",onto$name)
