...
calc_correlation <- function(cols_meta,expr_mat){
  corr_vector <- vector("numeric", nrow(expr_mat))
  for(i in 1:nrow(expr_mat)){
    corr_vector[i] = suppressWarnings(cor(cols_meta$age, expr_mat[i,]))
  }
  return(corr_vector)
}
...
for(i in 1:nrow(struct_to_ids)){ # for each unique brain structure
  ...
  if(nrow(temp_cols) > 1){
    corr_matrix[,i] = calc_correlation(temp_cols, temp_mat)
  }
}

# note some individual correlation values are NA
# error with "temporal neocortex" because only one sample - use if statement to test length of num cols
