...
temp_stat_df_model <- data.frame(
  ...
  corr_internal = numeric(length=num_features), # determine gene expression trajectory over time
  mean_external = numeric(length=num_features),
  sd_external = numeric(length=num_features),
  tau_external = numeric(length=num_features),
  corr_external = numeric(length=num_features) # determine gene expression trajectory over time
...
for(i in 1:length(temp_els)){
  ...
  brainspan <- onto$brainspan_cols[[temp_els[i]]]
