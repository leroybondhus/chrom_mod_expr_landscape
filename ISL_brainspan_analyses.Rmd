...
resolution_dirs <- c("high_res_cutoff_0.2/", "modest_res_cutoff_0.4/")
...
ggsave(paste0(dirs$figure_cluster_analysis, resolution_dirs[1], struct, "/", date, struct, "_subcluster_expression_over_time(cutoff=0.2).png"), plot = gg_high, width = 10, height = 8)
...
ggsave(paste0(dirs$figure_cluster_analysis, resolution_dirs[2], struct, "/", date, struct, "_subcluster_expression_over_time(cutoff=0.4).png"), plot = gg_modest, width = 10, height = 8)
...
modest_cluster_dirs <- paste0("cluster", 1:n_clusters_modest, "/")
high_cluster_dirs <- paste0("cluster", 1:n_clusters_high, "/")

for (dir in modest_cluster_dirs){
  if(!dir.exists(paste0(dirs$figure_cluster_analysis, resolution_dirs[2],struct, "/", dir))){dir.create(paste0(dirs$figure_cluster_analysis, resolution_dirs[2],struct, "/", dir))}
}

for (dir in high_cluster_dirs){
  if(!dir.exists(paste0(dirs$figure_cluster_analysis, resolution_dirs[1],struct, "/", dir))){dir.create(paste0(dirs$figure_cluster_analysis, resolution_dirs[1],struct, "/", dir))}
}
...
hist(cluster_max_expr[[3]], breaks = 50)
```
...
main_results_dir <- paste0(dirs$figure_cluster_analysis, resolution_dirs[2],struct, "/")
main_subdir_names <- str_replace_all(modest_cluster_dirs, "/", "")
gencode_genes <- gene_universe

test = 1

# chrom_gene_lists_modest <- chrom_gene_lists_modest[which(lengths(chrom_gene_lists_modest) > 1)]

# Iterate through the gene lists and perform gene enrichment analysis
# for (i in 1:length(chrom_gene_lists_modest)) {
foreach(i = 1:length(chrom_gene_lists_modest)) %dopar% {
  # if (length(chrom_gene_lists_modest[i]) == 1){test = 2}
  # else{
    ...
    for (ont in ontologies) {
        ...
        enrich_result <- enrichGO(gene = chrom_gene_lists_modest[[i]],
...
cluster_assignment <- vector("integer", nrow(sub_matrix)) # vector to store cluster assignments for each gene
...
for (i in 1:nrow(sub_matrix)){ # loop through all the genes
  ...
  cluster_assignment[i] = suppressWarnings(max(which(corr_vec >= 0.8), na.rm=T))
};rm(corr_vec)
