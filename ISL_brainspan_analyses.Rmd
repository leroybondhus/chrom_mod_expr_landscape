...
# Harold's formula: https://haroldpimentel.wordpress.com/2014/05/08/what-the-fpkm-a-review-rna-seq-expression-units/
rpkmToTpm <- function(rpkm)
{
    rpkm/sum(rpkm)*(1e6)
}
tpm1 = rpkmToTpm(expression_mat[,1])

converted_expression <- apply(expression_mat, 2, rpkmToTpm)
...
# list genes of interest (note BRPF2 not in rows_meta)
...
gene_list <- c("KAT6A", "KAT6B", "ING5", "BRPF1", "BRPF3", "MEAF6", "FOXG1")

gene_expression_plot <- function(gene){
  row <- which(rows_meta$gene_symbol == gene)
  expression <- sorted_expression[row,]
  pcw <- sorted_cols$age
  structures <- sorted_cols$structure_name
  df <- data.frame(structures, pcw, expression)
  ggplot(df, aes(x = log10(pcw), y = log10(expression+1), color = structures, group = structures)) + 
  ...
  ggsave(paste0(gene, " expression plot.png"), width = 7, height = 5) 
}

sapply(gene_list, gene_expression_plot)
```
