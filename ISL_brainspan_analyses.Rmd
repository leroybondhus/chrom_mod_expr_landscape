...
chromatin_modifiers <- gene_sets$human_chromatin_modifiers$gene_subsets$all_chrom_modifiers
...
struct = str_replace_all("amygdaloid complex", "[ /]", "_")
...
sub_matrix <- normal_expr[matched_modifiers, struct_index]
sub_matrix <- sub_matrix[which(apply(sub_matrix, 1, max)-apply(sub_matrix, 1, min)>0.5),]
...
# cut at a distance of 0.5, and get cluster memberships
myhcl <- cutree(hc, h=0.5)
n_clusters <- max(myhcl)

# create dataframe with cluster mean expression per time point
cluster_mean_expr <- vector("list", max(n_clusters))
for (i in 1:n_clusters){
  # error in colMeans if only 1 gene in a cluster so check length
  if(length(which(myhcl == i)) > 1){
    cluster_mean_expr[[i]] = colMeans(mean_expr[which(myhcl == i),])
  }
  else{
    cluster_mean_expr[[i]] = mean_expr[which(myhcl == i),]
  }
}

temp <- as.data.frame(table(myhcl))

test_df <- melt(cluster_mean_expr)
test_df$pcw <- sorted_cols$age[struct_index]
colnames(test_df)[1] = "expr"
colnames(test_df)[2] = "cluster"
test_df$freq <- temp$Freq[match(test_df$cluster, temp$myhcl)]
test_df$group_name <- paste0("cluster:",test_df$cluster, ",num_genes:", test_df$freq)

# see what these coexpression patterns look like 
library(ggforce)

# NEED TO FIX SO THAT FACETS IN CLUSTER ORDER TO ACCESS SUBPLOTS PROPERLY

gg <- ggplot(test_df, aes(x = log10(pcw), y = expr)) +
...
subplots <- vector(mode = "list", length = n_clusters)

# for (i in 1:n_clusters){
#   subplots[i] <- ggplot(test_df, aes(x = log10(pcw), y = expr)) +
#       geom_point() + geom_line() +
#       facet_wrap_paginate(group_name~., nrow = 1, ncol = 1, page = i) +
# theme(legend.position = "none") +
#       ggtitle("amygdaloid complex gene expression over time for each cluster") + 
#       labs(x = "log10(pcw)", y = "normalized expr") + 
#      geom_vline(xintercept = log10(40), color = "red") + geom_vline(xintercept = log10(52*15), color = "red") + geom_vline(xintercept = log10(52*10), color = "red")
...
subplots <- ggplot(test_df, aes(x = log10(pcw), y = expr)) +
      geom_point() + geom_line() +
