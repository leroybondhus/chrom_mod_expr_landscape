...
for(i in 1:length(temp_onto_names)){
  ...
  temp_df$pgt_p9[which(temp_df$pLI + temp_df$pRec > 0.9)] <- "pLI+pRec gt 0.9"
  temp_df$pgt_p9[which(temp_df$pLI > 0.9)] <- "pLI gt 0.9"
  temp_df$pgt_p9[which(temp_df$pRec > 0.9)] <- "pRec gt 0.9"
  temp_df$pgt_p9[which(temp_df$pNull > 0.9)] <- "pNull gt 0.9"
  ...
  ggplot(temp_df_1, aes(fill=paste(chrom_subset_name, is_syndr_associated), x=chrom_subset_name, y=count/sum,
                        label=count))+
    geom_col(position="stack")+
    theme_bw(base_size = 8)+
    geom_text(size=1.8, position = position_stack(0.5))+
    scale_y_continuous(breaks = seq(0,1,by=0.1)) +
    scale_fill_manual(values=c("honeydew2","#00BFC4","thistle1","#C77CFF"))+# "plum","grey50")))+
    theme(plot.title = element_text(size=5),
          axis.text.x = element_text(angle=90)) 
  filename <- paste0(dirs$figures,"syn_",temp_onto_names[i],".png")
  ggsave( filename,
          #gridExtra::grid.arrange(gg1,gg2,gg3, ncol=3, widths=c(1,2.5,2.5)),,
          width=4,height=4)
  ...
  ggplot(temp_df_2, aes(fill=paste(chrom_subset_name, both), x=chrom_subset_name, y=count/sum,
                        label=count))+
    geom_col(position="stack")+
    theme_bw(base_size = 8)+
    geom_text(size=1.8, position = position_stack(0.5))+
    scale_y_continuous(breaks = seq(0,1,by=0.1)) +
    scale_fill_manual(values=rev(c("lightblue3","#00BFC4","plum","#C77CFF")))+# "plum","grey50")))+
    theme(plot.title = element_text(size=5),
          axis.text.x = element_text(angle=90)) +
    facet_grid(. ~ dom_rec)
  filename <- paste0(dirs$figures,"rec_dom",temp_onto_names[i],".png")
  ggsave( filename,
          #gridExtra::grid.arrange(gg1,gg2,gg3, ncol=3, widths=c(1,2.5,2.5)),
          width=4,height=4)

  ggplot(temp_df[which(temp_df$is_syndr_associated),], aes(fill=dom_rec,x=""))+
  ...
  ggplggplot(temp_df, aes(fill=dom_rec,x=is_syndr_associated))+
    geom_bar()+
    theme_bw(base_size = 8)+
    scale_fill_manual(values=c("honeydew","mistyrose","lightblue","salmon"))+
    theme(plot.title = element_text(size=5),
    ...
    filename <- paste0(dirs$figures,"synd_pgt_p9",str_remove_all(temp_onto_names[i]," "),".png")
    ggsave(filename = filename,width = 4,height=4)
  
  # temp_df$pLI_p_pRec <- temp_df$pLI + temp_df$pRec
  # temp_df$pLI_gt_p9 <- temp_df$pLI > 0.9 
  # for(j in 1:length(c("pLI","pRec","pLI_p_pRec"))){
  #   temp_colname <- c("pLI","pRec","pLI_p_pRec")[j]
  #   temp_df$pLR_gt_p9 <- temp_df[temp_colname] > 0.9
  #   temp_df_1 <- aggregate(ensembl_gene_id ~  is_syndr_associated + pLR_gt_p9 + is_chrom_set + chrom_subset_name,
  #                        data=temp_df, FUN=length)
  #   colnames(temp_df_1)[which(colnames(temp_df_1)=="ensembl_gene_id")] <- "count"
  #   temp_df_1 <- merge(temp_df_1, setNames(aggregate(count ~ chrom_subset_name, data=temp_df_1, FUN=sum),c("chrom_subset_name","total")))
  #   ggplot(temp_df_1, aes(x=chrom_subset_name,y=count / total,
  #                         label=count, fill=paste0(pLR_gt_p9,chrom_subset_name )))+
  ...
  #     theme(plot.title = element_text(size=5),
  #           axis.text.x = element_text(angle=90))+
  #     facet_grid(paste0("is syndrome associated ", is_syndr_associated) ~ .)
  #   
  #   filename <- paste0(dirs$figures,temp_colname,"_synd_nosynd_chrom_nochrom_",str_remove_all(temp_onto_names[i]," "),".png")
  #   ggsave(filename = filename,width = 4,height=4)
  # }
}
...
ggplot(or_df[grepl("ubiq_chrom|^chrom$", or_df$chrom_subset_name) &
               grepl("^all$",or_df$contrast_w) &
               complete.cases(or_df) &  
               or_df$major_systems_affected != "" 
             ,], aes(y=major_systems_affected,x=log2(or),color=chrom_subset_name))+
...
filename <- paste0(dirs$figures,"or_ubiq_chrom.png")
...
or_df$contrast_w <- factor(or_df$contrast_w, levels=rev(names(temp_list)))
for(i in 1:length(temp_onto_names)){
  ...
  gg2 <- ggplot(temp[which((temp$chrom_subset_name==temp_onto_names[i] | 
  ...
  gg3 <- ggplot(or_df[temp_onto_names[i]==or_df$chrom_subset_name &
                 !grepl("all_|_chrom",or_df$contrast_w) &
                 complete.cases(or_df) &  
                 or_df$major_systems_affected != "" 
               ,], aes(y=major_systems_affected,x=-1*log2(or),color=contrast_w))+
    ggtitle(temp_onto_names[i])+
...
ggplot(temp[which((grepl("ubiq_chrom|nonubiq_chrom",temp$chrom_subset_name) | 
                    grepl("^all$",temp$chrom_subset_name)) & 
                    temp$major_systems_affected != ""),],
       aes(y=major_systems_affected, x=num_genes_w_sys_affected/num_genes_w_syndrome, fill=chrom_subset_name,
           label=num_genes_w_sys_affected))+
  ggtitle(temp_onto_names[i])+
  ...
  geom_vline(xintercept = 1)+
  geom_text(size=2, hjust=1.5, position=position_dodge(0.9))+
  scale_x_continuous(breaks = seq(0,1,by=0.1)) +
  theme_bw(base_size = 8)+
  theme(plot.title = element_text(size=5),
        strip.text.y = element_text(angle = 0),
        axis.text =  element_text(size=5))
filename <- paste0(dirs$figures,"sysphen_prop_ubiq_chrom.png")
ggsave(filename,width=5,height=4)
