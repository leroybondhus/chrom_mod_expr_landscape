...
for(i in 1:length(temp_onto_names)){
  ...
  gg2 <- ggplot(temp_df_2, aes(fill=paste(chrom_subset_name, both), x=chrom_subset_name, y=count/sum,
    ...
    geom_text(size=1.8, position = position_stack(0.5))+
    scale_y_continuous(breaks = seq(0,1,by=0.1)) +
    scale_fill_manual(values=c("#00BFC4","lightblue3","#C77CFF","plum"))+# "plum","grey50")))+
    theme(plot.title = element_text(size=5),
          axis.text.x = element_text(angle=90),
          legend.position = "none") +
    facet_grid(. ~ dom_rec)
  ...
  filename <- paste0(dirs$figures,"sysinhert_pp9",temp_onto_names[i],".png")
  ggsave( filename,
          gridExtra::grid.arrange(gg1,gridExtra::grid.arrange(gg3,gg2,nrow=3,heights=c(1,1,0.05)), ncol=2, widths=c(1,1.5)),
          width=6,height=8)
