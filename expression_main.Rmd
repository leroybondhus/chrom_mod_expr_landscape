...
for(i in 1:length(temp_onto_names)){
  ...
  temp_df <- unique(temp_df[,c("ensembl_gene_id","chrom_subset_name")])
  ...
  temp_df <- rbind(temp_df, data.frame(ensembl_gene_id=temp, chrom_subset_name=names(temp_list)[which],
                                       is_chrom_set=TRUE, is_syndr_associated=F))
  
  which <- grep(paste0("^",temp_system," ALL$"),names(temp_list))
  temp <- temp_list[[which]][which(!is.element(temp_list[[which]], temp_df$ensembl_gene_id))]
  temp_df <- rbind(temp_df, data.frame(ensembl_gene_id=temp, chrom_subset_name=names(temp_list)[which],
                                       is_chrom_set=FALSE, is_syndr_associated=F))
...
for(i in 1:length(temp_onto_names)){
  ggplot(or_df[temp_onto_names[i]==or_df$chrom_subset_name &
                 !grepl("all_|_chrom",or_df$contrast_w) &
                 complete.cases(or_df) &  
                 or_df$major_systems_affected != "" 
               ,], aes(y=major_systems_affected,x=log2(or),color=contrast_w))+
  ...
  filename <- paste0(dirs$figures,"or_",temp_onto_names[i],".png")
...
ggplot(or_df[!is.element(or_df$chrom_subset_name,c("all","chrom")) & 
               complete.cases(or_df) & 
               or_df$major_systems_affected != "",], aes(y=major_systems_affected,x=log2(or),color=contrast_w,group=contrast_w))+
  geom_errorbar(aes(xmin=log2(or_lb),xmax=log2(or_ub)),position=position_dodge(width=0.7), width=1)+
  geom_point(size=1.5,position=position_dodge(width=0.7))+
  geom_vline(xintercept = 0)+
  scale_x_continuous(breaks = -5:5)+
...
ggsave(filename,width=8,height=14)
...
for(i in 1:length(temp_onto_names)){
  ggplot(temp[which((temp$chrom_subset_name==temp_onto_names[i] | 
                      grepl("^all$|^chrom$",temp$chrom_subset_name)) & 
