...
for(assay in unique(dataset_list$ENCODE_HH$agg_med_norm__coldata$assay_term_name)){
  which <- which(dataset_list$ENCODE_HH$agg_med_norm__coldata == assay)
  temp_mat <- dataset_list$ENCODE_HH$agg_med_norm__mat[,which]
  temp_coldata <- dataset_list$ENCODE_HH$agg_med_norm__coldata[which,]
  ...
  ggplot(df, aes(x=wmean,y=name))+
    ...
    geom_point(data = sub_temp_mat, aes(x=expr, y = name, color=assay),size=0.1,alpha=0.4)+#,color="firebrick4")+
    geom_errorbar(alpha=0.8,color="grey20",aes(xmin=df$wmeanv_l,xmax=df$wmeanv_u))+
    theme(axis.text.y = element_text(size=2))
  ggsave(paste0(dirs$figures, assay, "__each_modifier_expression_dist.png"),dev="png",units = "in",width=8,height=10)
  ...
  filename <- paste0(dirs$figures, assay, "__expression_dist.png")
