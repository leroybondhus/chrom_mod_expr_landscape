...
#for(i in 1:nrow(query_table)){
  ...
  target <- httr::GET(target)
  target_result <- httr::content(target, "text", encoding = "UTF-8")
  target_result <- jsonlite::fromJSON(target_result, flatten = TRUE)
  ...
  which <- which(target_result$files$file_format=="tsv" &
                 ...
                 grepl(max(as.numeric(stringr::str_remove_all(temp_gene_anno,"[a-zA-Z]")),na.rm = T),
  ...
  if(all(is.na(target_coldata$life_stage_age))){
    ...
    target_coldata$life_stage_age <- paste0(unlist(stringr::str_extract_all(tolower(query_table$biosample_summary[i]),temp_pattern)),collapse = ",")
  }
  ...
  for(j in 1:nrow(target_coldata)){
    ...
    rownames(temp_list$TPM) <- stringr::str_remove(temp_data$gene_id,"\\..*")
  }
  temp_list
}
...
temp <- tempfile() 
download.file("https://storage.googleapis.com/gtex_analysis_v8/annotations/GTEx_Analysis_v8_Annotations_SampleAttributesDS.txt",
              temp)
