...
temp_els <- which(temp_num_els > 10)[1:2]
...
for(i in 1:length(temp_els)){
  ...
  for(set_and_complement in c(1,-1)){
    ...
    temp_mat <- mat[,ehh*set_and_complement]
    temp_coldata <- coldata[ehh*set_and_complement,]
    if(ncol(temp_mat)>0){
    ...
    if(set_and_complement==1){
      ...
      onto$ehh_summary_stats$mean_external[[temp_els[i]]] <- weighted_means
      onto$ehh_summary_stats$sd_external[[temp_els[i]]] <- weighted_sds
    }
