...
dataset_list$TEXP_GTEX$tpm_median_mat <- dataset_list$TEXP_GTEX$median_mat
...
temp <- tempfile()
temp_url <- "https://storage.googleapis.com/adult-gtex/long-read-data/v9/long-read-RNA-seq/quantification_gencode.counts.txt.gz"
temp <- tempfile()
download.file(temp_url,temp)

dataset_list$LR_GTEX$raw_reads_mat <- read.table( temp, skip=0, header = TRUE, sep = "\t")
rownames(dataset_list$LR_GTEX$raw_reads_mat) <- dataset_list$LR_GTEX$raw_reads_mat$transcript
dataset_list$LR_GTEX$raw_reads_mat$transcript <- NULL
unlink(temp); rm(temp)

View(dataset_list$LR_GTEX$raw_reads_mat[dataset_list$LR_GTEX$median_mat_rowinfo$transcript[dataset_list$LR_GTEX$median_mat_rowinfo$gene_name=="KAT6A"],])
...
dataset_list$LR_GTEX$median_mat <- apply(dataset_list$LR_GTEX$median_mat, 2, FUN=function(x){ x * 1e6 /sum(x) })
dataset_list$LR_GTEX$tpm_median_mat <- dataset_list$LR_GTEX$median_mat
dataset_list$LR_GTEX$median_mat <- apply(dataset_list$LR_GTEX$median_mat, 2, FUN=function(x){ log10(x+1) })
dataset_list$LR_GTEX$median_mat <- apply(dataset_list$LR_GTEX$median_mat, 2, FUN=function(x){ x/median(x[which(x>1e-6)]) })
save.image()
```
