...
onto$ehh <- vector("list", length(onto$id))
for(i in 1:length(temp_uberon_ids)){
  if(i %% 100 == 0){print(i)}
  if(is.na(temp_uberon_ids[i])){next}
  onto <- add_annotation(temp_uberon_ids[i], onto, "ehh", i,
...
for(onto_term_index in 1:length(onto_terms)){
  for(gene_set_name_index in 1:2){ #length(gene_set_names)){
...
temp_coldata <- dataset_list$ENCODE_HH$coldata 
df1 <- data.frame(biosample = temp_coldata$biosample_ontology.term_name[ehh_1],
                 weights = weights)
...
temp_coldata <- dataset_list$ENCODE_HH$coldata 
df2 <- data.frame(biosample = temp_coldata$biosample_ontology.term_name[ehh_2],
                 weights = weights)
...
temp_coldata <- dataset_list$ENCODE_HH$coldata 
df3 <- data.frame(biosample = temp_coldata$biosample_ontology.term_name[ehh_3],
                 weights = weights)
